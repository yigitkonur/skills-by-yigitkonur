...
Three failed fixes means the frame is wrong. This file defines what counts as a "failed fix," how to re-enter the prior phase cleanly, when to route out to a structured user-driven reframe pause, and how to write the handoff so the receiving skill has the context.
...
Fail 3 → STOP. Route to a structured user-driven reframe pause. The problem is architecture-shaped.
```
...
Route to a structured user-driven reframe pause with the handoff template below. Do not try a 4th fix.
...
Then re-run the Phase 1 workflow. The new symptom card should be measurably richer than the previous one. If it isn't — if re-opening produced the same symptom card as before — that's evidence the current pattern families are exhausted. Proceed to **Fail 3 immediately** (don't wait for a third fix attempt) and route to a structured user-driven reframe pause with the two-fails handoff template below. The 3-fails rule is the upper bound; hitting it earlier when the signal is clear is not a violation, it's a time-saver.

## Handoff format to a structured user-driven reframe pause

At Fail 3, dispatch to a user-driven reframe pause with this exact template. Drop it into the conversation or, on runtimes with an ask-user tool, into the initial prompt:
...
<one sentence: the question a user-driven reframe pause should investigate — e.g., "Is our session
store assumption still valid?" or "Should this code path exist at all?">

### Constraints / non-negotiables
<anything a user-driven reframe pause should know — deadlines, deployment constraints, user-facing
commitments>
```

a user-driven reframe pause picks up from "architectural question surfaced" — its Cynefin classifier will usually route this to Complex (unknown unknowns) and run the full 6-step interactive session. When it returns a recommendation, re-enter at Phase 2 with the new framing. Do not restart `debug-runtime` from Phase 1 — the Phase 1 card is still valid.
...
**Counter**: the re-open block takes 2 minutes. If it is skipped and fix #3 also fails, the session reaches minute 35, Fail 3, and routes to a user-driven reframe pause with incomplete context. The 2-minute re-open is the cheaper path even under outage pressure.
...
**Counter**: the senior's diagnosis is a *candidate mechanism*, not a *confirmed one*. If the experiment on X falsified it, X is out. Re-open Phase 2 with a genuinely different pattern family. If the senior pushes back, route to a user-driven reframe pause early — the architectural question is now social as well as technical.
...
| Routing to a user-driven reframe pause with only the most recent fix's context | Full handoff template above. All three fixes. |
| Restarting `debug-runtime` from Phase 1 after a user-driven reframe pause returns | No — re-enter at Phase 2 with the new framing. Infinite regress otherwise. |
