...
Present the completed review to the user in the conversation. **Do not submit it to GitHub** (via `gh pr review --submit` or MCP tools) unless the user explicitly asks the reviewer to submit or post it. This prevents accidental review submissions on the wrong PR or with unintended verdicts.
...
3. **Present the review to the user -- do not auto-submit to GitHub.** Unless the user explicitly says "submit", "post", or "publish", present the review as a chat message. This prevents accidental submissions on the wrong PR or with unintended severity.
...
> **Cross-reference:** See `references/dimensions/severity-guide.md` for verdict-to-severity mapping rules and `references/output/communication.md` for tone guidance within findings.
