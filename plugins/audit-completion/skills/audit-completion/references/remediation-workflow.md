...
**Do not stop, pause, or ask for confirmation mid-remediation unless a blocker is genuinely unresolvable.** This intentionally differs from blocker-heavy plan execution workflows that stop on every blocker. `audit-completion` is designed to maximize progress through a fixed backlog; halting on every friction point defeats that.
