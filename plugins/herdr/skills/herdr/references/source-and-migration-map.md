...
| `parallel-capacity.md`: `## 6. Serial Integration & Delivery Lifecycle` | Remote only | Merged | `serial-merge-and-conflicts.md` | Rebase on moving baseline; self-contained conflict engine preserving invariants. |
| `mission-briefs.md`: `## 1. Common Authority Block (All Roles)` | Differ in text | Retained | `mission-briefs.md` §1 | Common authority block with identity, coordinates, objectives, checks, and artifact-only return option. |
...
| `event-monitoring.md`: `## 7. Discovery, Bootstrap & Model Verification` | Identical | Retained | `event-monitoring.md` §7 | Restored: live coordinates via `--current`, model verification via TUI/process, 10-minute silent boundary. |
...
| `serial-merge-and-conflicts.md`: `## 1. Why Serial Integration?` | Remote only | Retained | `serial-merge-and-conflicts.md` §1 | Serial landing sequence preventing semantic regressions and merge races. |
| `serial-merge-and-conflicts.md`: `## 2. Step-by-Step Serial Landing Pipeline` | Remote only | Qualified | `serial-merge-and-conflicts.md` §2 | Rebase on authorized branch; delta review on changed SHA; standard push; no target checkout in worktree. |
| `serial-merge-and-conflicts.md`: `## 3. Self-Contained Merge Conflict Resolution Engine` | Remote only | Retained | `serial-merge-and-conflicts.md` §3 | 5-step engine: path-safe `git diff --name-only --diff-filter=U`, understand intent, reconcile, check, non-interactive continue. |
| `serial-merge-and-conflicts.md`: `## 4. Worktree Lifecycle, Herdr Defaults & Clean Teardown` | Remote only | Qualified / Merged | `lifecycle-and-cleanup.md` | Understanding Herdr defaults; zero-bloat teardown protocol consolidated into decoupled 3-stage lifecycle. |
...
| `herdr-primitives.md`: `## 6. Notification Primitives (herdr notification)` | Differ in text | Retained | `herdr-primitives.md` §4 | Desktop toast alerts on task or wave completion. |
...
| `orchestration-workflow.md`: `## 6. Step 4: Full-Job Teardown & Serial Merge` | Differ in text | Merged | `lifecycle-and-cleanup.md`, `serial-merge-and-conflicts.md` | Replaced rigid teardown with decoupled 3-stage lifecycle and serial rebase pipeline. |
| `orchestration-workflow.md`: `## 7. Step 5: Continuous Wave Advancement` | Differ in text | Merged | `ticket-decomposition-and-waves.md` §3 | Dynamic DAG advancement as prerequisites clear. |
...
| `recovery-and-safeguards.md`: `## 4. Subagent Model Tiering` | Installed only | **Rejected** / Replaced | `parallel-capacity.md` §2 | **Forced Flash Tiering Rejected**: Model and effort follow user instructions strictly; no speculative downgrades. |
| `recovery-and-safeguards.md`: `## 5. Server Daemon Safety` | Installed only | Merged | `stopped-agent-recovery.md` §5 | Prohibits `herdr server stop` or blanket kills during active sessions. |
...
| `serial-merge-and-conflicts.md`: `## 3. Merge Conflict Resolution Protocol` | Differ in text | Merged | `serial-merge-and-conflicts.md` §3 | 5-step conflict resolution engine: path-safe `git diff --name-only --diff-filter=U`, understand intent, reconcile hunks. |
| `serial-merge-and-conflicts.md`: `## 4. Post-Merge Teardown & Resource Cleanup` | Differ in text | **Rejected** / Replaced | `lifecycle-and-cleanup.md` | **Anti-Patterns Rejected**: `branch -D` fallback, global remote prune, and zero-worktree prune goal replaced with safe decoupled 3-stage lifecycle. |
