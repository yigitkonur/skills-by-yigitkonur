...
| `serial-merge-and-conflicts.md`: `## 3. Self-Contained Merge Conflict Resolution Engine` | Remote-herdr only (absent in installed-herdr) | Retained | `serial-merge-and-conflicts.md` §3 | 5-step engine: path-safe `git diff --name-only --diff-filter=U`, understand intent, reconcile, check, non-interactive continue. |
