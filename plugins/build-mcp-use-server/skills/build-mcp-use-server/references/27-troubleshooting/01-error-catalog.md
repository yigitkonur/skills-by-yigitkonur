...
| `406 Not Acceptable: Client must accept both application/json and text/event-stream` | A legacy compatibility POST omitted the mandatory dual `Accept` header. The request is rejected before its JSON-RPC body is parsed. | Add `Accept: application/json, text/event-stream`, or use the native modern `2026-07-28` request shape. | `references/22-validate/02-curl-handshake.md` |
