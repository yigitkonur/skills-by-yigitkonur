# Schemas: Standard Schema & Zod v4

*Read this when writing `inputSchema` and `outputSchema` for tools, resources, and prompts.*

mcp-use v2 uses Standard Schema v1 (the `StandardSchemaWithJSON` interface), which covers Zod v4, ArkType, Valibot, and others. This page focuses on Zod v4 as the canonical choice.

## Key Rules

1. **Use Zod v4** (not v3). Install `zod@4.x` if upgrading from v1.
2. **Every field must have `.describe()`** — the LLM uses these texts to understand and fill arguments.
3. **Use `.strict()` on top-level objects** — prevents the LLM from hallucinating extra fields.
4. **Keep schemas flat** — aim for ≤6 fields; split into multiple tools if needed.
...
server.tool(
  { name: "notify", inputSchema: z.object({ notification }) },
  async ({ notification }) => {
...
server.tool(
  { name: "list-items", inputSchema: pagination.and(filters) },
  async ({ page, limit, status }) => ({ content: [{ type: "text", text: "Listing..." }] })
);
```
...
`completable()` is for `server.prompt()` argument autocomplete. It is **not supported** in `server.tool()` schemas. For prompt argument autocomplete, wrap fields in `server.prompt()` definitions with `completable()`. For resource template URI variable autocomplete, use `complete:` on the resource template definition.
