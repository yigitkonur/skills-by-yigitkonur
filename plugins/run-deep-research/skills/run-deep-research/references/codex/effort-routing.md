...
- **Reddit-thread quote extraction:** N Reddit permalinks; one row per thread (top voted comment, sentiment, bias label). Use `extract-evidence` inside the codex prompt with attribution requirements (e.g. "Which comments carry the highest score, and what do they say verbatim with author?") and a fixed output schema.
