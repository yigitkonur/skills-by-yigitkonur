...
4. **Mandatory Prompt Prefix (`/teamwork-preview /herdr`)**: Every mission prompt dispatched to an AGY worker or reviewer MUST begin with `/teamwork-preview /herdr`. This forces explicit activation of the Herdr control plane and activates multi-role teamwork cognition (Architect, Implementer, QA), dramatically elevating code and test quality.
5. **Streaming Review & Callback Loop**: Implementers write back `status=DONE`. As soon as any worker reports done, Antigravity immediately opens Tab 2 (`review`) inside that worktree workspace without waiting for other lanes.
6. **Deep Review-and-Fix**: The reviewer audits exact commit SHAs with domain skills (`code-review`, `tdd`, `audit-completion`), authors test/bug patches directly, and posts GitHub PR approval.
7. **Serial Integration & Wave Advancement**: Rebase approved candidates serially onto moving `main`, resolve any merge conflicts via `resolving-merge-conflicts` principles, merge to `main`, and advance to the next wave until all waves complete.
...
# 2. Launch Implementer & Dispatch with Mandatory Prefix:
