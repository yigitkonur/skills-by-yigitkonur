...
- *runtime bug with a reproducible failure → use `debug-runtime`* (this skill is reasoning before execution; `debug-runtime` is iterative diagnosis with a runtime)
- *reviewing a PR or branch diff for merge → use `review-pr`*
- *evaluating received review feedback → use `review-feedback`*
...
| Ambiguous / post-`debug-runtime` runtime hypothesis (SenseMaking, bug variant) | `references/workflows/bug-tracing.md`, `references/foundations/evidence-and-falsification.md` |
...
- **Out to `debug-runtime`** when: the task is a runtime bug with a reproducible failure, **or** a `plan-tradeoff` session produces a testable runtime hypothesis.
- **Out to `audit-completion`** when: declared "done" but unsure what's actually verified.
- **Do NOT recommend `plan-tradeoff` as the next-step** (no infinite regress). If more thinking is needed, name the sub-topic and explicitly hand off.

### `debug-runtime` route-back handoff

When this session — especially one entered from `debug-runtime` after 3 failed fixes — produces a falsifiable runtime hypothesis, hand back to `debug-runtime` with: `symptom card / observed failure`, `failed-fix summary if known`, `selected hypothesis + mechanism`, `falsification experiment`, `narrow next diagnostic or fix boundary`, and `verification check`.
...
| [`references/workflows/bug-tracing.md`](references/workflows/bug-tracing.md) | SenseMaking (bug variant) | Ambiguous / post-`debug-runtime` runtime hypothesis framing |
