...
**Use**: `workflows/sense-making.md` for the generic version. `workflows/bug-tracing.md` only for ambiguous or post-`debug-runtime` runtime hypotheses that need a falsifier handoff; reproducible runtime failures route to `debug-runtime`. `workflows/recurring-issue.md` when point fixes haven't worked.
...
**`plan-tradeoff`'s role here**: this is a *design-thinking* workflow, not a runtime workflow. `plan-tradeoff` gives the agent the discipline to specify the trigger correctly *before* the watcher runs. The actual execution is implemented by hooks/automation.
...
**`plan-tradeoff`'s role here**: design the sequence + idempotency + error handling *before* the orchestration runs. The actual execution is implemented by workflow tooling (n8n, temporal, langgraph, etc.).
...
| User says… | Operation |
|---|---|
| "Should we…?", "Is X safe?", "What's wrong?", "Why does X fail?", "Evaluate Y", "Review Z" | Sense-Making (route reproducible runtime failures to `debug-runtime`) |
...
|---|---|---|
| "Investigate this bug, then write the fix" | Sense-Making only when the failure is ambiguous or post-`debug-runtime`; otherwise `debug-runtime` owns it | Generative Composition (then Self-Verification) |
