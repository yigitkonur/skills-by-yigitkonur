...
Operation: **Sense-Making** (`Op: SenseMaking`), systemic variant. For reproducible one-off bugs use `debug-runtime`; for ambiguous/post-debug bug hypotheses use `bug-tracing.md`; for generic research/judgment use `sense-making.md`.
...
- The bug is genuinely a reproducible one-off (no pattern across releases) → use `debug-runtime`
- The system is stable (no feedback dynamics) → use `debug-runtime`, or `bug-tracing.md` only for an ambiguous/post-debug hypothesis handoff
- You are at Fail #1 of the 3-fails rule in `debug-runtime` — don't escalate prematurely; keep debugging first
