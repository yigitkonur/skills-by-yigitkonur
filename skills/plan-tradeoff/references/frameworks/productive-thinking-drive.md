...
The full Productive Thinking Model (Tim Hurson) wraps a 6-step problem-to-execution arc around the core loop. Most of its value for `plan-tradeoff` is the **DRIVE** sub-framework for defining "what success looks like." Use the rest of the 6-step model only when the deliverable is a full execution plan, not just a recommendation.
