...
Interactive brainstorm is the user-in-the-loop mode inside `plan-tradeoff`.
...
- [ ] No infinite-regress recommendation (don't recommend `plan-tradeoff` Interactive as the next step)
...
| Pointing the next-step at "plan-tradeoff Interactive" again | Infinite regress. Name a concrete follow-on (a `build-*` skill, `run-issue-tree`, `review-self`, direct implementation) |
