...
# optimize-agentic-mcp — Audit, Optimize, Architect MCP Servers
...
- Enforcing TypeScript Clean Architecture layering on `mcp-use/server` → route to `apply-clean-mcp-architecture`
- CLI agent-readiness (not MCP) → route to `optimize-agentic-cli`
- Migrating SDK v1 code to v2 → route to `convert-mcp-server-sdk-v1-to-v2`
...
| [`convert-mcp-server-sdk-v1-to-v2`](../../../convert-mcp-server-sdk-v1-to-v2/) | Migrating existing SDK v1 code to v2 alpha | Targets the migration only |
...
| [`apply-clean-mcp-architecture`](../../../apply-clean-mcp-architecture/) | TypeScript `mcp-use/server` audit hits layer-boundary, folder-layout, or dependency-direction issues | Layer discipline; not a replacement for this agent-readiness audit |
| [`optimize-agentic-cli`](../../../optimize-agentic-cli/) | The interface should stay CLI-first or the task is CLI agent-readiness, not MCP | CLI contract work, not MCP protocol work |
