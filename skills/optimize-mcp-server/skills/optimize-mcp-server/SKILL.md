...
| [`build-mcp-sdk`](../../../build-mcp-sdk/) | Canonical path: `@modelcontextprotocol/sdk` v1.x, single package, Zod v3, widest client compatibility, stdio or HTTP | Manual transport/OAuth wiring; deprecated overloads still compile so easy to pick wrong API |
| [`build-mcp-sdk-v2`](../../../build-mcp-sdk-v2/) | Only for new greenfield projects willing to accept beta. v2 split packages (`@modelcontextprotocol/server`), Zod v4, ESM-only, Node 20+ | Community adoption still early (Q1 2026 release); server-side OAuth removed; no CJS |
