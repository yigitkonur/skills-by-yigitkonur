...
|   |   --> Install build-mcp-sdk
|   +-- Remote HTTP
|       +-- New project, HTTP-first, want modern DX --> mcp-use server (install build-mcp-use-server)
|       +-- Mixed stdio + HTTP, widest compatibility --> Official TS SDK v1 (install build-mcp-sdk)
|       +-- Willing to try beta, new project only    --> Official TS SDK v2 (read build-mcp-sdk-v2 SKILL.md first)
|
...
- Beta v2 framework API changes land → re-read `build-mcp-sdk-v2` SKILL.md; consider falling back to v1.
