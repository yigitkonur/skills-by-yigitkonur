...
### AP-17: Research without a visible trail
...
**Fix:** Show the source shortlist, per-skill notes with relative paths, and comparison table inline in conversation output — each at the step that produces it.
...
The anti-patterns above cover problems in the *content* of a skill. For problems in the *process* of authoring a skill — reference overload, output batching, tool assumption, path confusion, research scope creep, and discipline-skill failures — see `references/examples/anti-patterns-authoring.md` (AP-25 through AP-34).
