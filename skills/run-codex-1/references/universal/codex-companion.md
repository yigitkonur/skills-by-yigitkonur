...
Status: `rescue-detect.py` reads codex-companion job records under `<plugin-data>/state/<slug>-<hash>/jobs/<id>.json` for forensic correlation. For single-mode rescue, the dispatcher reads `entry.codex_thread_id` directly from the manifest and threads it into `run-single.sh` as `--resume-thread <id>` (run-codex-1.mjs:2210-2218; run-single.sh resume case). The runner translates this into `codex exec resume <id>` — context preserved without going through `task-resume-candidate`. The companion's `task-resume-candidate` surface remains forensic-only: read it via `node <skill-root>/scripts/codex-cc/codex-companion.mjs task-resume-candidate --json --cwd <workspace-root>` to enumerate resumable threads when the manifest lacks a `codex_thread_id` (e.g. hand-rolled manifests, runner crashed before capturing thread.started).
...
```
<plugin-data>/state/<slug>-<hash>/run-codex-1/manifest.json
```
