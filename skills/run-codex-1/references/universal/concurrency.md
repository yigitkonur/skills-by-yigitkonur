...
**review = 4.** Per-branch parallel. Above 4 saturates GitHub's GraphQL budget when later phases (e.g. PR creation via `review-self`) interact with the same auth.

## Soft gate

Raising `JOBS` above the mode default OR setting `JOBS > 20` (any mode) triggers a stderr warning and requires `--i-have-measured` flag at the dispatcher level. The justification is recorded at `manifest.policy.overrides.concurrency` (the canonical write location seeded by `seedManifest()` in `scripts/run-codex-1.mjs`):

> **Both surfaces enforce identically.** The dispatcher (`run-codex-1.mjs:resolveConcurrency`) refuses raised concurrency without `--i-have-measured`. The bash runners `run-fleet.sh` and `run-batch.sh` apply the same gate (verified at `run-fleet.sh:169-176` and `run-batch.sh:148-155`) and exit 3 above 20 unless `ORCHESTRATE_CONCURRENCY_JUSTIFICATION` is set (or `--i-have-measured "<reason>"` is passed to the runner directly). Both layers refuse unconditionally above 100. The earlier doc framing of "WARN-only" was wrong. Bypassing the dispatcher does not bypass the gate; it just gives you a less ergonomic error surface (a runner stderr line instead of a typed envelope).
...
4. If 503s persist after halving twice, the auth tier is the bottleneck — no concurrency tuning fixes it. Wait longer, pause sibling Claude / run-codex-1 sessions sharing the same auth tier, or switch tiers. See `references/universal/failure-modes.md` row 1 for the multi-session auth-sharing note.
...
`JOBS > 100` is refused unconditionally. The skill assumes a single user's auth tier; concurrency above 100 is structurally pathological for this skill. If you genuinely need it, you're not running run-codex-1 — you're running a queue worker.
