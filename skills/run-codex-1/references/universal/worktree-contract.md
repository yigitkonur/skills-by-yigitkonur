...
The `<mode>` token in the path makes provenance obvious in `git worktree list`. A future user (or audit script) sees `myrepo-wt-exec-search-rewrite` and knows immediately which run-codex-1 run produced it.
...
Custom codegen: configure via `<source-repo>/.run-codex-1.yaml` (path TBD; not yet implemented; per-repo override mechanism deferred).
