...
The manifest is the single source of truth for an run-codex-1 run. Every mode reads from it; every status transition writes to it. It is the input to rescue mode and the audit gate for tidy.
...
```
resolveStateDir(cwd)/run-codex-1/manifest.json
```
...
Plugin-data resolution is shared across run-codex-1 (dispatcher + bash runners) and the vendored codex-companion (`scripts/codex-cc/lib/state.mjs`). Both follow the same chain:

1. If `${CLAUDE_PLUGIN_DATA}` is set, the state root is `${CLAUDE_PLUGIN_DATA}/state/<workspace-slug>-<hash>/`. The run-codex-1 manifest lives at `${CLAUDE_PLUGIN_DATA}/state/<workspace-slug>-<hash>/run-codex-1/manifest.json`.
2. If `${CLAUDE_PLUGIN_DATA}` is unset, the state root is `${TMPDIR:-/tmp}/codex-companion/<workspace-slug>-<hash>/`. The run-codex-1 manifest lives at `${TMPDIR:-/tmp}/codex-companion/<workspace-slug>-<hash>/run-codex-1/manifest.json`.
...
```json
{
  ...
  "state_dir": "/abs/state/<slug-hash>/run-codex-1",
...
The dispatcher (`scripts/run-codex-1.mjs:64-69`) seeds `concurrency_cap` from this table when the user does not pass `--concurrency`:
...
- The orchestrator owns the state, not the project. Multiple repos share the same run-codex-1 skill but each has its own state.
