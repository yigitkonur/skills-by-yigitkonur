...
- `post_verify_cmd` is wired: `buildExecEntries` in `scripts/run-codex-1.mjs` threads it onto `mode_state.post_verify_cmd` (and `mode_state.exec.post_verify_cmd` for back-compat); `run-fleet.sh` prefers it over the auto-detect table below. Omit it to fall back to the auto-detect recipe per language.
...
FAIL  02-cache-eviction (codex exit=1; runtime=42s; see <worktree>/.run-codex-1/02-cache-eviction.log)
