...
Two distinct behaviors based on whether `--apply` is passed (`handleRescue` in `scripts/run-codex-1.mjs:1879-2120`):
...
Subsets accepted by `--apply`: `failed-only` | `never-started-only` | `all-non-done` | `ids:s1,s2,...`. The CLI also accepts `--redo {failed,never-started,all-non-done}` as an alias — it normalizes into `--apply` (`run-codex-1.mjs:1889-1906`). `--apply` is the canonical form (matches the envelope's `rerun_with` template).
...
- A prior run-codex-1 run ended with non-terminal entries.
- The user explicitly invokes rescue: `node run-codex-1.mjs rescue [--manifest <path>]`.
...
```
node run-codex-1.mjs rescue [--manifest /abs/path/to/manifest.json]
node run-codex-1.mjs rescue --manifest /abs/path/to/manifest.json --apply failed-only
node run-codex-1.mjs rescue --manifest /abs/path/to/manifest.json --apply never-started-only
node run-codex-1.mjs rescue --manifest /abs/path/to/manifest.json --apply all-non-done --accept-stale
node run-codex-1.mjs rescue --manifest /abs/path/to/manifest.json --apply ids:02-config-editor,04-alert-fsm
```
...
Each match lives at `<state-root>/<slug>-<sha256-prefix>/run-codex-1/manifest.json`. The `<slug>` is the basename of the original workspace root, so you can usually identify the project by reading the directory name. Pass the chosen path to `--manifest <abs-path>` directly — bypasses the cwd-based resolver.
...
```bash
node run-codex-1.mjs rescue --manifest <path> --apply failed-only
node run-codex-1.mjs rescue --manifest <path> --apply never-started-only
node run-codex-1.mjs rescue --manifest <path> --apply all-non-done --accept-stale
```
...
When invoked with `--apply`, after pre-rescue cleanup and the queued-flip, `handleRescue` spawns the original mode's runner detached, with mode-appropriate args/env (`run-codex-1.mjs:2038-2101`). The skip-existing guards in each runner leave `done` entries alone; only the freshly-`queued` ones run.
