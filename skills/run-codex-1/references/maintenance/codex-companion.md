# codex-companion — vendored dispatcher (maintenance reference)

> Runtime rescue-correlation surface lives at `references/universal/codex-companion.md`. This document covers the vendored subtree's library tour, dispatcher rationale, and "why we don't drive review/exec/batch through codex-companion" — content a maintainer doing an upstream sync needs, but a runtime agent does not.
>
> See also `references/maintenance/upstream-codex-cc.md` for the vendoring + update procedure.
...
The skill's `run-codex-1.mjs` (the top-level dispatcher) imports three modules from the vendored tree:
...
Replicating these by hand would drift from upstream over time; importing keeps us aligned. See `references/maintenance/upstream-codex-cc.md` for update procedures.
