...
Every observable change in the run-codex-1 unification refactor (branch
`feat/run-codex-1-unification-263c3eec`). Backward-compat preserved
...
The pre-refactor audit baseline is at `scripts/__fixtures__/baseline/audit-pass-v1.md` (copy of `/tmp/run-codex-1-audit/MASTER-REPORT.md`, 87 CRITICAL findings). Re-running the same 22-agent audit pass against the refactored docs should reduce this toward 0; that's the v2.0-final acceptance bar.
