#!/usr/bin/env bash
# preflight.sh — universal pre-flight checks for run-codex-1 (renamed from bootstrap.sh).
#
# What it does:
...
#      run-codex-1 subdir. Manifest path is printed on stdout for the
...
STATE_DIR="$STATE_ROOT/$ORCHESTRATE_SLUG/run-codex-1"
...
# run-codex-1's worktree convention is `../<repo>-wt-<mode>-<slug>`,
...
# The dispatcher (run-codex-1.mjs) reads this block to populate the
