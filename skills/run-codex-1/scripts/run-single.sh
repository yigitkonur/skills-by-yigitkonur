...
# run-codex-1.mjs:2210-2219. Default unset = fresh `codex exec`.
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    # run-codex-1.mjs records the schema path in the manifest entry
    ...
    # Rescue context-preserving resume. run-codex-1.mjs:2210-2218 picks one
...
if [[ -z "$PROMPT_FILE" && -n "$PROMPT_TEXT" ]]; then
  mkdir -p "$CWD/.run-codex-1"
  PROMPT_FILE="$CWD/.run-codex-1/${ORCHESTRATE_ENTRY_ID}.prompt.md"
...
if [[ -z "$OUT" ]]; then
  OUT="$CWD/.run-codex-1/single.last.md"
fi
if [[ -z "$JSONL" ]]; then
