#!/usr/bin/env python3
"""audit.py — read-only inspection of run-codex-1 state.
...
def cmd_state(args: argparse.Namespace) -> int:
    ...
    manifest_path = (
        ...
        if args.manifest
        else resolve_state_dir(workspace_root) / "run-codex-1" / "manifest.json"
    )
    ...
    if not manifest_path.is_file():
        ...
        report["recommendations"].append(
            "Run `run-codex-1 audit --manifest <path>` with the expected manifest location, "
            "or seed a new run via `run-codex-1 <mode>`."
        )
...
def build_argparser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="audit.py",
        description="Read-only inspection of run-codex-1 state (state | sizes | worktrees).",
    )
