...
| `ORCHESTRATE_MANIFEST` | none | Path to the run-codex-1 manifest. Without it, the monitor only reports git/process counts (no manifest summary). |
| `MONITOR_ROOT` | `/tmp/run-codex-1-monitor` | Logs dir; `monitor.log` and `baseline.sha` land here |
