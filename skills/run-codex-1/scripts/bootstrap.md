...
|---|---|---|
| `MONITOR_ROOT` (env) | derived from `resolveStateDir(cwd)/run-codex-1/logs` | where logs and per-entry artifacts go |
...
```
STATE_DIR=/abs/path/.../state/myrepo-abc1234567890def/run-codex-1
MANIFEST_PATH=/abs/path/.../state/myrepo-abc1234567890def/run-codex-1/manifest.json
MONITOR_ROOT=/abs/path/.../state/myrepo-abc1234567890def/run-codex-1/logs
