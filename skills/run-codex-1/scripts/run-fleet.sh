...
run_one() {
  ...
  # run logs dir) and fall back to the worktree's .run-codex-1 dir if
  ...
  if [[ -n "$monitor_root" ]]; then
    ...
    base_dir="$wt_resolved/.run-codex-1"
  fi
  ...
  if [[ "$DRY_RUN_LOCAL" == "1" ]]; then
    ...
    # carryForwardDoneEntries (run-codex-1.mjs) to skip this entry on
