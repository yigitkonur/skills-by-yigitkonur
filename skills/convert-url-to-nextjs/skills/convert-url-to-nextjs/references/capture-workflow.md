...
| Network/runtime metadata | Browser helper eval/network/runtime inspection | stylesheet/script/font/image URLs, build IDs, framework data |
| No browser tool available | Block live capture; use saved snapshots or enable a browser driver | `capture-summary.md` explaining why live capture could not run |
