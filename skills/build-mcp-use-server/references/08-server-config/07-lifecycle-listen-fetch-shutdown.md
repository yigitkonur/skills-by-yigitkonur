...
**v2.0.0-beta.66 does NOT expose a shutdown API in `MCPServer`.** 

No `server.close()` or `server.shutdown()` method is documented or available in the public API. The underlying MCP handler (created by `createMcpMount()`) has a `.close()` method, but it is not exposed through MCPServer.

**Current workaround for Node processes:**
1. Let the process exit naturally (no active listeners remain)
2. Use OS signals (SIGTERM) to terminate the process
...
See `references/10-sessions/01-overview-stateless-truth.md` for session details.
...
Since v2 has no graceful shutdown API, focus on:
...
See `references/08-server-config/06-custom-routes.md` for health endpoint examples.
