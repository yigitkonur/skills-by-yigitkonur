...
This works for `listen()` (Node binds socket). For `server.fetch()`, localhost protection applies when the request's `Host` header matches localhost-class names.
...
See `references/08-server-config/05-middleware.md` for middleware patterns.
...
See `references/08-server-config/03-cors-and-allowed-origins.md` for Origin validation details.
