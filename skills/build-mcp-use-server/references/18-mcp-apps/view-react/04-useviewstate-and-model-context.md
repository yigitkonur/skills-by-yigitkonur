...
**Signature:**
```typescript
useViewState<T = Record<string, unknown>>(initialState: T | () => T): [T, (state: SetStateAction<T>) => void]
```
...
// Add an entry
modelContext.set("selected-product", "Wireless Headphones ($99)");

// Remove an entry
modelContext.remove("selected-product");
```

Use imperative when the state does not align with component hierarchy (e.g., multiple floating tooltips).
