...
> - **Track it in git** — add `!.mcp-use/project.json` to your `.gitignore` so the deployment link is available on every machine and CI runner. Without it in version control, deploying from a different machine or CI creates a new subdomain and breaks any custom domain (CNAME) you've configured.
> - Keep the rest of `.mcp-use/` gitignored (sessions, type registry, etc.)
...
**Use `--name`** to label your deployment: `mcp-use deploy --name my-server`. Note: the URL subdomain is auto-generated by Manufact Cloud (e.g., `black-silence-ot8sz`) and does NOT match `--name`. The `--name` is a label stored in `.mcp-use/project.json`. The URL is stable across redeployments as long as `project.json` is preserved.
...
5. If you have a custom domain (CNAME), verify it still resolves: `curl -s https://your-domain.com/health`

### Custom domains

If you CNAME a domain (e.g., `api.example.com`) to your Manufact Cloud deployment URL, the deployment link in `.mcp-use/project.json` is critical. Losing this file (deletion, new machine without it, CI without it in git) creates a new deployment with a new subdomain — your CNAME then points to a dead URL.

To protect custom domains:
1. Track `project.json` in git: add `!.mcp-use/project.json` to `.gitignore`
2. Never delete `.mcp-use/project.json` unless intentionally creating a new deployment
3. After any deploy, verify your custom domain still works
