...
**Why:** Linters and formatters handle this. Review attention should stay on what machines cannot catch.
...
**Instead:** If a reviewer sees a systemic issue, note it as a 💡 Question: "Has the team considered [X]? This might be worth a separate discussion."
...
**Instead:** Only flag if the reviewer can describe a concrete, realistic scenario. "If a user submits an empty array here, this will throw on line 45" is concrete.
...
**Instead:** If the review agrees, add a brief note on the existing thread: "Agree with @reviewer — this is important."
...
**Don't:** "You should refactor this" / "tests need to be added."
**Why:** Directive language creates defensiveness and assumes the reviewer knows the context better than the author.
...
**Why:** Approval signals to other reviewers and the author that the code is safe.
**Instead:** If the reviewer can't do a thorough review, say "I reviewed [X cluster] but didn't review [Y]."
...
**Why:** CI failures may indicate fundamental problems that would change the review.
...
**Instead:** If more than 15 issues appear, step back. Either the PR is too large (suggest splitting) or the review is being too noisy (re-apply the actionability filter).
...
**Don't:** "This will crash on null input" when it is unclear whether the input can be null.
**Why:** If the finding is wrong, the reviewer loses credibility for future reviews.
...
3. **Anti-pattern #13 (too many findings) often signals scope creep.** If 15+ findings appear, pause and re-apply the actionability gate from SKILL.md Phase 6 before continuing. Consider whether the PR itself is too large rather than continuing to enumerate problems.
...
> **Cross-reference:** Load `references/output/communication.md` for phrasing alternatives when the review drifts into anti-pattern language.
