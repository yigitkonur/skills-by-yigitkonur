...
Read-only GitHub PR context collector for `do-review`. It gathers the same metadata, file list, checks, commits, formal reviews, inline comments, and conversation comments that the skill needs before diff review.
...
```bash
jq '.[] | {filename, additions, deletions}' /tmp/do-review-*/files.json
jq '.[] | {author: .user.login, state, body}' /tmp/do-review-*/reviews.json
```
