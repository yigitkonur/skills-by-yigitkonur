...
node <skill-root>/scripts/run-codex-2.mjs --help
...
The dispatcher `run-codex-2.mjs` imports these explicitly. After an update:
1. Re-run `node --check <skill-root>/scripts/run-codex-2.mjs` — catches stale imports.
...
- Identify the replacement; update the import in `run-codex-2.mjs`.
