...
The manifest is the single source of truth for a run-codex-2 run. Every mode reads from it; every status transition writes to it. It is the input to rescue mode and the audit gate for tidy.
...
```
resolveStateDir(cwd)/run-codex-2/manifest.json
```
...
Plugin-data resolution is shared across run-codex-2 (dispatcher + bash runners) and the vendored codex-companion (`scripts/codex-cc/lib/state.mjs`). Both follow the same chain:

1. If `${CLAUDE_PLUGIN_DATA}` is set, the state root is `${CLAUDE_PLUGIN_DATA}/state/<workspace-slug>-<hash>/`. The run-codex-2 manifest lives at `${CLAUDE_PLUGIN_DATA}/state/<workspace-slug>-<hash>/run-codex-2/manifest.json`.
2. If `${CLAUDE_PLUGIN_DATA}` is unset, the state root is `${TMPDIR:-/tmp}/codex-companion/<workspace-slug>-<hash>/`. The run-codex-2 manifest lives at `${TMPDIR:-/tmp}/codex-companion/<workspace-slug>-<hash>/run-codex-2/manifest.json`.
...
```json
{
  ...
  "state_dir": "/abs/state/<slug-hash>/run-codex-2",
...
| `policy.model` | string | Yes | `gpt-5.5` default. Override via env `USE_CODEX_CODEX_MODEL`. |
| `policy.effort` | string | Yes | `xhigh` default. Override via env `USE_CODEX_CODEX_EFFORT`. |
...
The dispatcher (`scripts/run-codex-2.mjs:64-69`) seeds `concurrency_cap` from this table when the user does not pass `--concurrency`:
...
- The orchestrator owns the state, not the project. Multiple repos share the same run-codex-2 skill but each has its own state.
