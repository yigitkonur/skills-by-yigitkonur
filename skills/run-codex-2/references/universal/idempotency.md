...
**Implemented.** A dispatcher convenience flag `--force-redo <slug>[,<slug2>,...]` is wired in `run-codex-2.mjs` for `exec` and `batch` modes. It archives the existing answer file (batch) to `<answers>/.prev/<slug>-<ts>.md`, flips the entry's status back to `queued`, and re-spawns the runner.

```bash
# Force re-run of one entry (exec or batch):
node scripts/run-codex-2.mjs <mode> --force-redo <slug>

# Force re-run multiple entries (comma-separated; one --force-redo flag):
node scripts/run-codex-2.mjs <mode> --force-redo "slug-a,slug-b"

# Force re-run every entry:
node scripts/run-codex-2.mjs <mode> --force-redo-all
```

Each `--force-redo` cycle increments `attempts` by 2 (once on the queued-flip in `flipEntryToQueued` at `run-codex-2.mjs:1101`, once on runner START in `run-batch.sh:238`). This is a known artifact of the two-phase manifest write — the bookkeeping double-counts but does not affect skip-existing or rescue classification. When auditing `attempts` after one or more force-redos, divide by 2 to get the operator-visible cycle count.
...
- Pass `--force-new-run --run-id <custom>` to write a sibling manifest (`manifest.<custom-run-id>.json` in the same state directory). **Implemented** in `run-codex-2.mjs` for exec/batch/single. The runners would still race on per-entry locks if both runs target overlapping entries; use this only when the new run touches a disjoint task set.
