...
Monitor({
  ...
  command: "USE_CODEX_MANIFEST=<manifest-path> USE_CODEX_QUIET_AFTER=1 bash <skill-root>/scripts/codex-monitor.sh | grep -E --line-buffered '(streak:[0-9]+x|agent-done-committed|manifest-changed|fleet-quiet|^--- fleet quiet ---)'",
  persistent: true,
  timeout_ms: 300000
})
```
...
- `USE_CODEX_QUIET_AFTER=1` (or any value ≥ 1) is required for the script to emit the `--- fleet quiet ---` terminal sentinel; the default (0) means the script never auto-exits. See "Per-mode Monitor commands → exec mode" below.
...
```bash
USE_CODEX_MANIFEST=<manifest> USE_CODEX_QUIET_AFTER=1 \
...
- `[flag1,flag2]` — action signals. Canonical flag vocabulary (verified at `scripts/codex-monitor.sh:196,199,203,208`):
  ...
  - `fleet-quiet` — manifest is all-terminal AND `procs=codex:0`. When `USE_CODEX_QUIET_AFTER>0`, this flag also drives auto-exit (see Stopping a Monitor below).
...
```bash
USE_CODEX_MANIFEST=<manifest> MONITOR_ROOT=<dir> bash <skill-root>/scripts/codex-monitor.sh
```
...
```bash
jq '.entries[] | {id, status, round: .mode_state.review.round}' "$USE_CODEX_MANIFEST"
```
...
- **exec / review** (`codex-monitor.sh`): `--- fleet quiet ---` is emitted on stdout **only when** `USE_CODEX_QUIET_AFTER` is set to a positive integer AND the fleet has been quiet for that many consecutive ticks (see `scripts/codex-monitor.sh:43, 231-233`). Default is `0` = never auto-exit. Set `USE_CODEX_QUIET_AFTER=1` (or higher to debounce) when you want a terminal sentinel; otherwise rely on the `fleet-quiet` flag inside per-tick lines and `TaskStop` from the agent.
...
- `USE_CODEX_QUIET_AFTER=N` — fleet-quiet ticks before the loop emits `--- fleet quiet ---` and exits (default 0 = never auto-exit).
- `MONITOR_ROOT=<dir>` — log directory (default `/tmp/run-codex-2-monitor`).
