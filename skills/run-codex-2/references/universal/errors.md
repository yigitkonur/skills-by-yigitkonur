# Error code catalog — `node run-codex-2.mjs`
...
Source of truth: `scripts/run-codex-2.mjs` — every `errEnvelope(...)` call site plus the `EXIT_CODE_BY_ERROR` map (lines 134-178). Helper-internal codes (Python helpers, bash runners) are not catalogued here unless they bubble through the dispatcher envelope.
...
|---|---|---|---|---|
| `unknown_mode` | Subcommand is not one of `exec\|batch\|single\|review\|rescue\|audit\|tidy\|help`. | 2 | Re-run with a valid subcommand. `node run-codex-2.mjs help`. | SKILL.md "Detect the mode" |
...
| `manifest_not_found` | `rescue` / `audit` / `tidy` got no manifest at the resolved path. The manifest may have been tidied, or `--manifest <path>` is wrong. | 3 | Pass an explicit `--manifest <abs-path>`, or enumerate siblings with `ls "$(run-codex-2 --resolve-state-dir)/manifest"*.json`. | `idempotency.md#enumerating-manifest-siblings` |
...
| `codex_unauthenticated` | `bootstrap.sh` saw `codex login status` exit non-zero (status 2 or 3 from the script). Codex spawns would fail immediately. | 5 | Run `codex login`; re-invoke. Bypass with `USE_CODEX_SKIP_CODEX_AUTH=1` only when using bearer-token / managed-companion / proxy auth where `login status` reports false-negative. | SKILL.md preflight step 2 |
...
Any unmapped error code falls back to **exit 1** (see `EXIT_CODE_BY_ERROR` default in `run-codex-2.mjs:178`).
...
node scripts/run-codex-2.mjs rescue --manifest <path>

# Run in parallel without disturbing the live run (sibling manifest):
node scripts/run-codex-2.mjs <mode> --force-new-run --run-id <custom> ...
```
...
node scripts/run-codex-2.mjs rescue --manifest <path> --accept-stale
```
...
# Normal fix:
codex login
node scripts/run-codex-2.mjs <mode> ...

# Bypass for bearer-token / managed / proxy auth:
USE_CODEX_SKIP_CODEX_AUTH=1 node scripts/run-codex-2.mjs <mode> ...
```
