...
PROMPTS=p ANSWERS=a LOGS=l JOBS=N MIN_BYTES=N \
  USE_CODEX_MANIFEST=<path> bash run-batch.sh
```

| Flag | Env | Default | Notes |
|---|---|---|---|
| `--manifest <path>` | `USE_CODEX_MANIFEST` | unset | Path to run-codex-2 manifest. Required for manifest writes; absent is a valid no-manifest mode. |
