...
function scenarioExec() {
  ...
  expect(parsed.command.includes("USE_CODEX_MANIFEST="),
    "command exports USE_CODEX_MANIFEST");
...
function scenarioCarryForwardDoneEntries() {
  ...
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "run-codex-2-carry-forward-"));
...
function scenarioUnknownCommand() {
  ...
  const r = spawnSync(process.execPath, [dispatcher, "not-a-real-mode"], {
    encoding: "utf8",
    env: { ...process.env, USE_CODEX_SKIP_PREFLIGHT: "1" },
...
async function main() {
  process.stdout.write("test-monitor-integration.mjs — run-codex-2 Monitor + dispatcher harness\n");
  scenarioExec();
  scenarioBatch();
  scenarioSingle();
  scenarioReview();
