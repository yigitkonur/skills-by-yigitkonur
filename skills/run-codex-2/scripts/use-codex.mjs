#!/usr/bin/env node
// run-codex-2.mjs — top-level dispatcher for the run-codex-2 skill.
//
...
const MODE_USAGE = {
  exec: [
    "Usage: node run-codex-2.mjs exec --tasks <tasks.json> [args]",
    "",
    "Required:",
  ...
  batch: [
    "Usage: node run-codex-2.mjs batch --inputs <file> --template <tmpl> [args]",
    "",
    "Required:",
  ...
  single: [
    "Usage: node run-codex-2.mjs single (--prompt <text> | --prompt-file <file>) [args]",
    "",
    ...
    "Optional:",
    "  --cwd <dir>                      workspace cwd (default: process.cwd)",
    "  --out <file>                     codex -o final-message file (default: <cwd>/.run-codex-2/single.last.md)",
  ...
  review: [
    "Usage: node run-codex-2.mjs review --branches <list-or-file> [--base <ref>] [args]",
    "",
    "Required:",
    ...
    "Usage: node run-codex-2.mjs rescue [--manifest <path>] [--cwd <dir>] [--apply <subset>]",
    "",
    "Optional:",
  ...
  audit: [
    "Usage: node run-codex-2.mjs audit [--manifest <path>] [--cwd <dir>] [--json]",
    "",
    "Optional:",
  ...
  tidy: [
    "Usage: node run-codex-2.mjs tidy [--manifest <path>] [--cwd <dir>] [--execute] [--json]",
    "",
    "Optional:",
    "  --manifest <path>                explicit manifest",
  ...
  if (unknownLong.length > 0) {
    return {
      err: errEnvelope(command, "unknown_option",
        ...
        `Run \`node run-codex-2.mjs help\` for the supported flags.`,
...
function manifestPathFor(cwd, runId = null) {
  // Matches codex-companion's state dir + a run-codex-2/ subdir so our
  ...
  const base = path.join(resolveStateDir(cwd), "run-codex-2");
  return runId
...
function buildMonitorForMode(mode, ctx) {
  ...
  switch (mode) {
    ...
    case "review":
      return {
        tool_hint: buildMonitorHint({
          description: `run-codex-2 ${mode} (run_id=${runId})`,
    ...
    case "single":
      return {
        tool_hint: buildMonitorHint({
          description: `run-codex-2 single (run_id=${runId})`,
...
function dispatchHelp() {
  return okEnvelope("help", {
    ...
    usage: [
      "Usage: node run-codex-2.mjs <mode> [args]",
      "",
      "Modes:",
...
async function handleRescue(argv) {
  ...
  if (!options.apply) {
    ...
    const env = okEnvelope(command, {
      ...
      next_action: {
        ...
        rerun_with: `node run-codex-2.mjs rescue --manifest ${manifestPath} --apply <subset>`,
      },
      manifest_path: manifestPath,
      run_id: m.run_id,
      mode: m.mode,
  ...
  if (dryRun) {
    const rerunCmd = `node run-codex-2.mjs rescue --manifest ${manifestPath} --apply ${String(options.apply)}`;
    const env = okEnvelope(command, {
      phase: "preview",
      next_action: {
        kind: "preview",
