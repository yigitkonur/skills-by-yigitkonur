...
if [[ "${USE_CODEX_CODEX_FLAGS_LOADED:-}" == "1" ]]; then
  return 0 2>/dev/null || exit 0
fi
USE_CODEX_CODEX_FLAGS_LOADED=1

# Per-session overrides (from env). The skill documents these in
# references/universal/codex-flags.md.
CODEX_MODEL="${USE_CODEX_CODEX_MODEL:-gpt-5.5}"
CODEX_EFFORT="${USE_CODEX_CODEX_EFFORT:-xhigh}"
