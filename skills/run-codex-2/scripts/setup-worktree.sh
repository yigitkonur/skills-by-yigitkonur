#!/usr/bin/env bash
# setup-worktree.sh — create a git worktree for a run-codex-2 worker,
...
#   USE_CODEX_MODE   exec|batch|single|review|rescue (default: exec)
...
USE_CODEX_MODE="${USE_CODEX_MODE:-exec}"
...
if [[ -n "${WORKTREE_DIR:-}" ]]; then
  ...
  WT_PATH="$PARENT/${REPO_NAME}-wt-${USE_CODEX_MODE}-${SLUG}"
fi
...
echo "[setup-worktree] mode=$USE_CODEX_MODE slug=$SLUG branch=$BRANCH_NAME base=$BASE_BRANCH"
