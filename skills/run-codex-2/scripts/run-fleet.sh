...
#   USE_CODEX_MODE  exported to setup-worktree.sh as `exec`
#
...
MANIFEST="${USE_CODEX_MANIFEST:-}"
ONLY_IDS="${ONLY_IDS:-}"
CONCURRENCY_JUSTIFICATION="${USE_CODEX_CONCURRENCY_JUSTIFICATION:-}"
export USE_CODEX_MODE="${USE_CODEX_MODE:-exec}"
...
# task worktrees) still see the same file via $USE_CODEX_MANIFEST.
case "$MANIFEST" in
  /*) ;;
...
run_one() {
  ...
  # run logs dir) and fall back to the worktree's .run-codex-2 dir if
  # monitor_root is missing.
  local log_path jsonl_path answer_path monitor_root run_id base_dir
  log_path="$(jq -r --arg id "$id" '.entries[] | select(.id == $id) | .log_path // ""' "$USE_CODEX_MANIFEST")"
  jsonl_path="$(jq -r --arg id "$id" '.entries[] | select(.id == $id) | .jsonl_path // ""' "$USE_CODEX_MANIFEST")"
  answer_path="$(jq -r --arg id "$id" '.entries[] | select(.id == $id) | .answer_path // ""' "$USE_CODEX_MANIFEST")"
  monitor_root="$(jq -r '.monitor_root // ""' "$USE_CODEX_MANIFEST")"
  run_id="$(jq -r '.run_id // ""' "$USE_CODEX_MANIFEST")"
  if [[ -n "$monitor_root" ]]; then
    ...
    base_dir="$wt_resolved/.run-codex-2"
  fi
  ...
  if [[ "$DRY_RUN_LOCAL" == "1" ]]; then
    ...
    # carryForwardDoneEntries (run-codex-2.mjs) to skip this entry on
  ...
  # Parse the root thread_id from the JSONL stream. Subagent threads can
  # appear first, so ignore thread.started events with parent/subagent markers.
  ...
  if [[ "$POST_VERIFY" == "1" ]]; then
    ...
    pv_cmd="$(jq -r --arg id "$id" '
      ...
      | (.mode_state.post_verify_cmd
         ...
         // "")
    ' "$USE_CODEX_MANIFEST" 2>/dev/null || echo "")"
    if [[ -z "$pv_cmd" ]]; then
