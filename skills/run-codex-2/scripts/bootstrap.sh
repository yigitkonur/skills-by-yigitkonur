#!/usr/bin/env bash
# bootstrap.sh — one-shot universal pre-flight for run-codex-2.
#
# What it does:
...
#      run-codex-2 subdir. Manifest path is printed on stdout for the
...
#   USE_CODEX_MODE  exec|batch|single|review|rescue (default: unknown)
#   USE_CODEX_RUN_ID  run id; if empty, generated and emitted on stdout
#   USE_CODEX_SLUG    workspace slug override (defaults to derived from cwd)
...
USE_CODEX_MODE="${USE_CODEX_MODE:-unknown}"
SKIP_GIT="${SKIP_GIT:-0}"
USE_CODEX_RUN_ID="${USE_CODEX_RUN_ID:-}"
USE_CODEX_SLUG="${USE_CODEX_SLUG:-}"
...
say "mode=$USE_CODEX_MODE project=$PROJECT_DIR"
...
if codex login status >/dev/null 2>&1; then
  ...
  if [[ "${USE_CODEX_SKIP_CODEX_AUTH:-0}" == "1" ]]; then
    say "WARN: codex login status returned non-zero; USE_CODEX_SKIP_CODEX_AUTH=1 bypassed the hard gate."
  else
...
STATE_DIR="$STATE_ROOT/$USE_CODEX_SLUG/run-codex-2"
...
# ── 5. Run id ─────────────────────────────────────────────────
if [[ -z "$USE_CODEX_RUN_ID" ]]; then
  if command -v openssl >/dev/null 2>&1; then
    suffix="$(openssl rand -hex 2)"
  else
    suffix="$(printf '%04x' $(( RANDOM & 0xffff )))"
  fi
  USE_CODEX_RUN_ID="$(date -u +%Y%m%dT%H%M%SZ)-$suffix"
fi
say "run-id: $USE_CODEX_RUN_ID"

# ── 6. .gitignore advisory for the worktree pattern ───────────
# run-codex-2's worktree convention is `../<repo>-wt-<mode>-<slug>`,
...
# The dispatcher (run-codex-2.mjs) reads this block to populate the
...
RUN_ID=$USE_CODEX_RUN_ID
PLUGIN_DATA_ROOT=$PLUGIN_DATA_ROOT
WORKSPACE_SLUG=$USE_CODEX_SLUG
