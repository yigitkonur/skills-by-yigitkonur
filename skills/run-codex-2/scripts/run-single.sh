...
#   USE_CODEX_MANIFEST + USE_CODEX_ENTRY_ID for status writes
...
USE_CODEX_MANIFEST="${USE_CODEX_MANIFEST:-}"
USE_CODEX_ENTRY_ID="${USE_CODEX_ENTRY_ID:-single}"
REUSE_WORKTREE=0
USE_CODEX_OUTPUT_SCHEMA="${USE_CODEX_OUTPUT_SCHEMA:-}"
# Rescue redispatch flags — set by --resume-thread / --resume-last from
# run-codex-2.mjs:2210-2219. Default unset = fresh `codex exec`.
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --manifest)    USE_CODEX_MANIFEST="$2"; shift 2 ;;
    --entry-id)    USE_CODEX_ENTRY_ID="$2"; shift 2 ;;
    ...
    # run-codex-2.mjs records the schema path in the manifest entry
    ...
    --output-schema) USE_CODEX_OUTPUT_SCHEMA="$2"; shift 2 ;;
    # Rescue context-preserving resume. run-codex-2.mjs:2210-2218 picks one
...
if [[ -z "$PROMPT_FILE" && -n "$PROMPT_TEXT" ]]; then
  mkdir -p "$CWD/.run-codex-2"
  PROMPT_FILE="$CWD/.run-codex-2/${USE_CODEX_ENTRY_ID}.prompt.md"
...
if [[ -z "$OUT" ]]; then
  OUT="$CWD/.run-codex-2/single.last.md"
fi
if [[ -z "$JSONL" ]]; then
  JSONL="$(dirname "$OUT")/${USE_CODEX_ENTRY_ID}.jsonl"
fi
...
SCHEMA_TAIL=()
if [[ -n "${USE_CODEX_OUTPUT_SCHEMA:-}" ]]; then
  SCHEMA_TAIL=(--output-schema "$USE_CODEX_OUTPUT_SCHEMA")
fi
...
LOG_PATH="$(dirname "$OUT")/${USE_CODEX_ENTRY_ID}.jsonl"
ERR_LOG="$(dirname "$OUT")/${USE_CODEX_ENTRY_ID}.err.log"
mkdir -p "$(dirname "$LOG_PATH")" "$(dirname "$ERR_LOG")"

if [[ -n "$USE_CODEX_MANIFEST" && -f "$USE_CODEX_MANIFEST" ]]; then
  "$SCRIPT_DIR/manifest-update.sh" entry "$USE_CODEX_MANIFEST" "$USE_CODEX_ENTRY_ID" \
...
if [[ "$DRY_RUN" == "1" ]]; then
  ...
  printf '{"type":"run-codex-2.done","entry_id":"%s","status":"done"}\n' "$USE_CODEX_ENTRY_ID" >> "$LOG_PATH"
  echo "DONE  $USE_CODEX_ENTRY_ID (dry-run)"
...
# Sentinel writer: appends one run-codex-2.done JSONL event to the live log
...
emit_orchestrate_done() {
  local status="$1"
  if [[ -n "$LOG_PATH" ]]; then
    printf '{"type":"run-codex-2.done","entry_id":"%s","status":"%s"}\n' \
      "$USE_CODEX_ENTRY_ID" "$status" >> "$LOG_PATH" 2>/dev/null || true
  fi
}

if [[ "$EXIT_CODE" -ne 0 ]]; then
  echo "FAIL  $USE_CODEX_ENTRY_ID (codex exit=$EXIT_CODE; runtime=${ELAPSED}s; see $ERR_LOG)"
  if [[ -n "$USE_CODEX_MANIFEST" && -f "$USE_CODEX_MANIFEST" ]]; then
    "$SCRIPT_DIR/manifest-update.sh" entry "$USE_CODEX_MANIFEST" "$USE_CODEX_ENTRY_ID" \
...
if [[ ! -s "$OUT" ]]; then
  echo "FAIL  $USE_CODEX_ENTRY_ID (codex exit=0 but answer empty: $OUT)"
  if [[ -n "$USE_CODEX_MANIFEST" && -f "$USE_CODEX_MANIFEST" ]]; then
    "$SCRIPT_DIR/manifest-update.sh" entry "$USE_CODEX_MANIFEST" "$USE_CODEX_ENTRY_ID" \
...
echo "DONE  $USE_CODEX_ENTRY_ID (runtime=${ELAPSED}s; answer=$(wc -c < "$OUT" | tr -d ' ')B)"
if [[ -n "$USE_CODEX_MANIFEST" && -f "$USE_CODEX_MANIFEST" ]]; then
  "$SCRIPT_DIR/manifest-update.sh" entry "$USE_CODEX_MANIFEST" "$USE_CODEX_ENTRY_ID" \
