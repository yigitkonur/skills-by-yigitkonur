...
Two invocation shapes. The manifest-aware shape is preferred for any run dispatched through `node scripts/run-codex-2.mjs batch` because it picks up the dispatcher's `--answers-dir <override>` automatically; the positional shape is preserved for direct/standalone use.
