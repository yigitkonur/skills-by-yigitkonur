#!/usr/bin/env python3
"""Classify run-codex-2 manifest entries for rescue-mode operator review.
...
Read-only. Outputs JSON for the dispatcher (run-codex-2.mjs) or human
...
`run-codex-2.mjs rescue --redo failed|never-started|all-non-done`.
...
def main() -> int:
    ...
    ap.add_argument("--manifest", required=True, help="Path to run-codex-2 manifest.json")
    ...
    ap.add_argument("--json", action="store_true", help="Emit JSON output for run-codex-2.mjs.")
    args = ap.parse_args()
