...
const DISPATCHER = path.join(SCRIPT_DIR, "run-codex-2.mjs");
...
function tmpdir(name) {
  return fs.mkdtempSync(path.join(os.tmpdir(), `run-codex-2-contract-${name}-`));
}
...
function runNode(args, cwd, extraEnv = {}) {
  const r = spawnSync(process.execPath, [DISPATCHER, ...args], {
    ...
    env: {
      ...process.env,
      NODE_ENV: "test",
      USE_CODEX_SKIP_PREFLIGHT: "1",
      USE_CODEX_RUNNER_FOREGROUND: "1",
      ...extraEnv,
    },
...
function scenarioSingle() {
  ...
  // T-ADD-6: foreground_completed contract — under USE_CODEX_RUNNER_FOREGROUND=1
...
// at run-codex-2.mjs:1474 persists `answers_dir` to manifest.answers_dir
...
function scenarioRescueAnswersDir() {
  ...
  // foreground spawn path's stdout dump (USE_CODEX_RUNNER_FOREGROUND=1).
  const redo = runNode([
    "rescue", "--cwd", cwd,
...
function scenarioSentinelParity() {
  ...
  // the run-codex-2.done JSONL into `--- single done (single: done) ---`.
  ...
  if (sLog && fs.existsSync(sLog)) {
    ...
    // Verify the run-codex-2.done sentinel was appended to JSONL for filter translation
    const jsonlPath = sOut.json?.result?.jsonl_path;
    if (jsonlPath && fs.existsSync(jsonlPath)) {
      const j = fs.readFileSync(jsonlPath, "utf8");
      expect(j.includes('"type":"run-codex-2.done"'),
        "single runner appends run-codex-2.done to JSONL for filter translation");
    }
  } else {
...
function scenarioSymlinkInvocationInline() {
  ...
  const real = path.join(SCRIPT_DIR, "run-codex-2.mjs");
...
// run-codex-2.mjs handleRescue (between corruption and freshness gates)
