...
#   --manifest <m> / USE_CODEX_MANIFEST=m   path to manifest.json for status writes
...
# dispatcher (run-codex-2.mjs spawnRunnerDetached) is the canonical
...
run_one() {
  ...
  if [[ -s "$answer" && "$DRY_RUN" != "1" ]]; then
    echo "SKIP  $name"
    if [[ -n "$USE_CODEX_MANIFEST" && -x "$SCRIPT_DIR_ABS/manifest-update.sh" ]]; then
      local prior_status=""
      ...
      if [[ "$prior_status" == "queued" || -z "$prior_status" ]]; then
        "$SCRIPT_DIR_ABS/manifest-update.sh" entry "$USE_CODEX_MANIFEST" "$name" \
...
export CODEX_FLAGS_STR SCRIPT_DIR_ABS ANSWERS LOGS DRY_RUN USE_CODEX_MANIFEST MIN_BYTES ONLY_IDS_LOCAL="$ONLY_IDS"
export -f run_one
