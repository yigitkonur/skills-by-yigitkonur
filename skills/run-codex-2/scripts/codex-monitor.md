...
```bash
USE_CODEX_MANIFEST=<path> bash codex-monitor.sh
```
...
| `USE_CODEX_MANIFEST` | none | Path to the run-codex-2 manifest. Without it, the monitor only reports git/process counts (no manifest summary). |
| `MONITOR_ROOT` | `/tmp/run-codex-2-monitor` | Logs dir; `monitor.log` and `baseline.sha` land here |
...
| `NOTIFY_TITLE` | `Use Codex` | osascript title |
...
| `USE_CODEX_QUIET_AFTER` | `0` | Legacy: ticks of consecutive `fleet-quiet` before emitting `--- fleet quiet ---` and exiting. Kept for backward compatibility; superseded by `AUTOSTOP_GRACE_SEC`. |
...
- `--- fleet quiet ---` — legacy sentinel emitted when `USE_CODEX_QUIET_AFTER>0` and that many consecutive `fleet-quiet` ticks have elapsed.
