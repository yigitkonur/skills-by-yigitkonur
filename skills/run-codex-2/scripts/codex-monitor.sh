...
#   USE_CODEX_MANIFEST         manifest.json path (recommended; without it
...
#                                `streak` (default: 3)
#   NOTIFY_TITLE                 osascript title (default: "Use Codex")
...
#   USE_CODEX_QUIET_AFTER      ticks of `procs=0 && all-terminal` before
...
manifest_summary() {
  if [[ -z "$USE_CODEX_MANIFEST" || ! -f "$USE_CODEX_MANIFEST" ]]; then
  ...
  ' "$USE_CODEX_MANIFEST" 2>/dev/null || printf 'manifest=parse-error'
}
...
manifest_terminal_state() {
  ...
  local total
  total="$(jq -r '(.entries // []) | length' "$USE_CODEX_MANIFEST" 2>/dev/null || echo "0")"
  if [[ "$total" == "0" ]]; then
    printf 'in-flight'
...
echo "[startup $(date -u +%H:%M:%SZ)] manifest=${USE_CODEX_MANIFEST:-none} interval=${INTERVAL}s baseline=${BASELINE:0:8} autostop_grace=${AUTOSTOP_GRACE_SEC}s"
...
while true; do
  ...
  # 2. Legacy sentinel: USE_CODEX_QUIET_AFTER>0 fires `--- fleet quiet ---`
