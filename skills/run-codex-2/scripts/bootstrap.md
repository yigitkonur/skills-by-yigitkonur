...
|---|---|---|
| `MONITOR_ROOT` (env) | derived from `resolveStateDir(cwd)/run-codex-2/logs` | where logs and per-entry artifacts go |
...
```
STATE_DIR=/abs/path/.../state/myrepo-abc1234567890def/run-codex-2
MANIFEST_PATH=/abs/path/.../state/myrepo-abc1234567890def/run-codex-2/manifest.json
MONITOR_ROOT=/abs/path/.../state/myrepo-abc1234567890def/run-codex-2/logs
