...
const server = new MCPServer({
  ...
  streamManager: new RedisStreamManager({
    ...
    prefix: "mcp:stream:",      // channel prefix (default)
    heartbeatInterval: 10,      // seconds (default: 10)
  }),
});
```

**How it works:** Client connects to Server A → SSE stream created, Server A subscribes to `mcp:stream:{sessionId}`. Client's next request hits Server B → Server B publishes notification to Redis → Server A receives it → pushes via SSE to the client.
