...
| Executing tool | `true` | `false` | Last partial value | `{}` |
...
3. mcp-use intercepts these partial arguments and delivers them to the widget's `useWidget()` hook as `partialToolInput`.
4. Once the LLM finishes generating arguments, your server tool handler executes normally and returns the final result.

The server never sees partial arguments — it only runs after the full tool input is assembled. Everything that powers `partialToolInput` and `isStreaming` on the client happens at the protocol layer.

### Example: A Normal Tool Handler

```typescript
import { tool, widget } from "mcp-use";
import { z } from "zod";
...
const { isStreaming, partialToolInput, isPending, props } = useWidget<MyProps>();
```

- **`isStreaming`** — `true` while the LLM is actively generating tool arguments
- **`partialToolInput`** — A `Partial<TProps>` object that grows as more tokens arrive. Fields appear one at a time as the LLM generates them.
- **`isPending`** — `true` during both streaming and server execution
- **`props`** — Empty during streaming; full data after server responds
...
const ArticlePreview: React.FC = () => {
  const { props, isPending, isStreaming, partialToolInput } = useWidget<ArticleProps>();
  ...
  return (
    <article className="p-4">
      <h1 className="text-2xl font-bold">
        {data?.title ?? <span className="text-gray-400">Untitled</span>}
      </h1>
      {data?.author && (
