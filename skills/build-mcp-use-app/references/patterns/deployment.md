...
```bash
npm install -g @mcp-use/cli   # included with create-mcp-use-app
mcp-use login
mcp-use deploy
```
...
```bash
mcp-use deploy --name my-server --env DATABASE_URL=postgres://... --env-file .env.production --open
```
...
```json
{
  "mcpServers": {
    "cloud-run": {
      "url": "https://mcp-server-PROJECT_NUMBER.REGION.run.app/mcp",
...
```bash
supabase secrets set MCP_URL="https://YOUR_REF.supabase.co/functions/v1/mcp-server" --project-ref YOUR_REF
...
# Or manual deploy (Docker required for static files)
npm run build
