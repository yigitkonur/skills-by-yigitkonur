...
export const widgetMetadata = {
  title: "System Dashboard", description: "Displays server health metrics",
  schema: z.object({ refreshInterval: z.number().default(30).describe("Refresh interval in seconds") }),
};
export default function Dashboard({ refreshInterval }: { refreshInterval: number }) {
  return (<div><h2>System Dashboard</h2><p>Auto-refresh every {refreshInterval}s</p></div>);
}
```
