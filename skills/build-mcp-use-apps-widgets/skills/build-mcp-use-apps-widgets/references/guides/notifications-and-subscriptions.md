...
Definitive guide to sending notifications, tracking progress, managing resource subscriptions, and receiving client events with mcp-use.
...
|--------|-----------|---------|-------------|
| `server.sendNotification` | `sendNotification(method: string, params?: any): Promise<void>` | `Promise<void>` | Sends a notification to **all** connected clients |
| `server.sendNotificationToSession` | `sendNotificationToSession(sessionId: string, method: string, params?: any): Promise<boolean>` | `Promise<boolean>` | Sends a notification to a **specific** session; returns `true` if delivered |
| `server.sendToolsListChanged` | `sendToolsListChanged(): Promise<void>` | `Promise<void>` | Convenience wrapper for `notifications/tools/list_changed` |
| `server.sendResourcesListChanged` | `sendResourcesListChanged(): Promise<void>` | `Promise<void>` | Convenience wrapper for `notifications/resources/list_changed` |
| `server.sendPromptsListChanged` | `sendPromptsListChanged(): Promise<void>` | `Promise<void>` | Convenience wrapper for `notifications/prompts/list_changed` |
| `server.getActiveSessions` | `getActiveSessions(): string[]` | `string[]` | Returns array of active session ID strings |
| `server.listRoots` | `listRoots(sessionId: string): Promise<Root[] \| null>` | `Promise<Root[] \| null>` | Requests current roots from a specific client |
| `server.onRootsChanged` | `onRootsChanged(cb: (roots: Root[]) => void): void` | `void` | Registers handler for client root change notifications |
| `server.notifyResourceUpdated` | `notifyResourceUpdated(uri: string): Promise<void>` | `Promise<void>` | Notifies subscribers that a resource has changed |

### Context (`ToolContext`) Method Signatures

| Method | Signature | Returns | Description |
|--------|-----------|---------|-------------|
| `ctx.sendNotification` | `sendNotification(method: string, params?: any): Promise<void>` | `Promise<void>` | Sends a notification to **the calling client** only |
| `ctx.sendNotificationToSession` | `sendNotificationToSession(sessionId: string, method: string, params?: any): Promise<boolean>` | `Promise<boolean>` | Sends to a specific session from within a tool handler |
| `ctx.reportProgress` | `reportProgress(progress: number, total?: number, message?: string): Promise<void>` | `Promise<void>` | Emits a `notifications/progress` notification |
...
Target a single client by session ID. Returns `true` on success, `false` if the session is not found or expired:
...
Cross-session notification from within a tool handler:
...
> Only clients that have subscribed to the URI receive this notification.
...
### Server-Side: Defining a Subscribable Resource
...
### Client-Side: Subscribing to a Resource

```typescript
import { MCPClient } from "mcp-use";

const client = new MCPClient({
  mcpServers: { "config-server": { url: "http://localhost:3000/mcp" } },
});

async function run() {
  const session = await client.createSession("config-server");

  // Subscribe to the configuration resource
...
|--------|-----------|-------------|
| `session.connector.subscribeToResource` | `(uri: string): Promise<void>` | Subscribe to updates for a resource URI |
| `session.connector.unsubscribeFromResource` | `(uri: string): Promise<void>` | Remove subscription for a resource URI |
| `session.connector.readResource` | `<T = any>(uri: string): Promise<T>` | Read current value of a resource |
| `session.on` | `(event: "notification", listener: (n: Notification) => void): void` | Listen for server-sent notifications |
...
## Complete Example — Dynamic Tool Registration with Notification
...
// Register for client root changes
server.onRootsChanged(async (roots) => {
  console.log(`Client roots updated: ${roots.length} root(s)`);
  roots.forEach((root) => console.log(`  - ${root.name || "unnamed"}: ${root.uri}`));
});

// Dynamic tool — behavior changes based on mode
server.tool(
  { name: "ping-pong", description: "Responds with current mode (ping or pong)" },
  async () => ({
    content: [{ type: "text", text: `${currentMode.toUpperCase()}!` }],
  })
);

// Toggle mode and notify all clients
server.tool(
  {
    name: "toggle-mode",
    description: "Toggle between ping and pong mode, notify clients to refresh tools",
  },
  async () => {
    ...
    return {
      content: [{
        ...
        text: `Mode toggled from "${oldMode}" to "${currentMode}"! ` +
              `${server.getActiveSessions().length} client(s) notified.`,
      }],
    };
  }
);
...
| `ctx.sendNotificationToSession(id, method, params)` | One client | Cross-session notification from tool handler |
