...
| `_meta` | `Record<string, unknown>` | No | Opaque metadata passed through to MCP protocol (e.g., `openai/outputTemplate`) |
| `widget` | `ToolWidgetConfig` | No | Widget config for tools returning `widget()` responses; **required** when returning `widget()` |
...
## Widget Response Data Flow

When returning `widget()`, the tool result has three fields with different visibility:
...
return widget({
  ...
  // Alternatively: output: text(`Weather in ${city}: ${temperature}°C`)
});
```

> `message` and `output` are equivalent ways to set the LLM-visible text. `message` is a convenience shorthand for `output: text(message)`.
...
async (args, ctx) => {
  ...
  ctx.log("info", "Processing...", "my-logger"); // Optional third arg: logger name
...
| `ctx.client.supportsApps()` | MCP Apps extension | Returns `true` if client supports MCP Apps (SEP-1865) |
| `ctx.client.extension(id)` | MCP Apps extension | Returns extension metadata by ID (e.g., `'io.modelcontextprotocol/ui'`), or `undefined` |
...
|---|---|---|
| `subject` | `string` | Stable opaque user ID (same across conversations) |
| `conversationId` | `string` | Current chat thread ID (changes per chat) |
| `locale` | `string` | BCP-47 locale, e.g. `"it-IT"` (detected at session start) |
...
> `ctx.client.user()?.locale` is set server-side at session start. Inside a widget, prefer `useWidget().locale` — it reads the client-side value directly, which is fresher and browser-aware.
