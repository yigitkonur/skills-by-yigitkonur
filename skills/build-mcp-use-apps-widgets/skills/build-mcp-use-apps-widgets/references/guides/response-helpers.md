...
server.tool(
  ...
  async ({ city }) => {
    ...
    return widget({
      ...
      // Use `message` for a plain-text LLM summary, or `output` for a full helper result
      message: `Current weather in ${city}: ${w.temp}°C, ${w.conditions}`,
...
> **`message` vs `output`:** Use `message` (string) for a quick text summary. Use `output` (a helper result like `text()` or `object()`) when you need a structured or rich-text fallback. Both end up in `content` for the LLM.
...
Helpers that return structured data — especially `object()` and `array()` — return a typed result. That generic type lets TypeScript know what the server is returning so your tool handlers stay self-documenting.
...
// Type parameter is optional — TypeScript infers it from the argument
