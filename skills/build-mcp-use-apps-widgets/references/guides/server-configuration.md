...
| `sessionStore` | `SessionStore` | `InMemorySessionStore` | Pluggable session storage backend (`InMemorySessionStore`, `FileSystemSessionStore`, `RedisSessionStore`) |
| `streamManager` | `StreamManager` | `InMemoryStreamManager` | SSE stream manager for push notifications (`InMemoryStreamManager`, `RedisStreamManager`) |
...
## 10. Session Storage Options

Three session store implementations are available, importable from `mcp-use/server`:

| Store | Use Case | Notes |
|---|---|---|
| `InMemorySessionStore` | Default; single-instance | Sessions lost on server restart |
| `FileSystemSessionStore` | Development | Sessions survive HMR reloads; not for production |
| `RedisSessionStore` | Production; multi-instance | Requires `npm install redis`; enables horizontal scaling |

### InMemorySessionStore (default)

No configuration needed. Suitable for single-instance production servers.

### FileSystemSessionStore (development)
...
const server = new MCPServer({
  ...
  sessionStore: new FileSystemSessionStore({
    ...
    maxAgeMs: 86400000,              // session TTL in ms (default: 24h = 86400000)
  }),
})
```

Sessions survive HMR reloads — clients won't need to re-initialize when you save a file. Not designed for production or distributed deployments.
...
Sessions persist across restarts and are shared across all instances, enabling horizontal scaling.
...
| `MCP_URL` | Full public base URL (overrides `baseUrl` config; used for widget/asset URLs and CSP) | `http://{HOST}:{PORT}` |
