...
4. Default to local files for meaningful `src/*`, `apps/*`, `services/*`, or `packages/*` subtrees identified as gap folders during Phase 0, but skip or collapse folders with no distinct workflow, risk, or convention.
...
- relevant Phase 0 scan and the audit findings
...
| One-wave exploration | Writing after Phase 0 scan only | Complete the audit first |
