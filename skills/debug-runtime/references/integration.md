...
This skill coordinates with two siblings in the pack: `plan-tradeoff` (which now covers both Solo deep-think and Interactive brainstorm modes — the former separate `do-brainstorm` skill is merged into `plan-tradeoff` Interactive mode) and `audit-completion` (audit-then-remediate). The inline routing matrix in SKILL.md covers the common cases; this file covers edge cases and the handoff templates.
...
- **Stuck on evidence** (Phase 2 pattern can't form) → route to `plan-tradeoff` (Solo)
- **Stuck on architecture** (3 fixes failed) → route to `plan-tradeoff` (Mode: Interactive)
...
## Route to `plan-tradeoff` (Solo) when...

`plan-tradeoff` Solo mode is the pack's tool-agnostic solo-reasoning mode. It does not require user input and applies a stronger evidence-and-falsification loop than Phase 2/3 of this skill.
...
- Phase 1 is incomplete (no 10/10 repro). `plan-tradeoff` can't fix an unreproducible bug; go back to Phase 1.
- Phase 3 experiment was falsifying and a candidate was confirmed. A mechanism exists; go to Phase 4.
- At the 3-fails gate. That's `plan-tradeoff` Interactive mode territory, not Solo.

### Worked handoff examples

**Example A** — Phase 2 candidates all weak:

> Handing to `plan-tradeoff` (Mode: Solo). Phase 1 is solid (symptom card + 10/10 repro). Phase 2 produced three candidate mechanisms, all rated "maybe": (1) race on the session store, (2) middleware order issue, (3) cache invalidation timing. None has evidence stronger than "it could be this." Need a stronger evidence frame before running Phase 3.

**Example B** — Phase 3 tied:

> Handing to `plan-tradeoff` (Mode: Solo). Two candidates fit: (1) connection pool exhaustion under load, (2) DNS resolution timing out on the cold path. Both would produce the observed "intermittent slow requests + error log." Tests so far have not distinguished them. Need a design for a falsifying experiment.

### Re-entry after `plan-tradeoff` Solo

Re-enter this skill at **Phase 2** with the sharper evidence frame. Do not re-run Phase 1.

## Route to `plan-tradeoff` (Mode: Interactive) when...

`plan-tradeoff` Interactive mode is the user-in-the-loop variant. It requires the user to participate at 5 forks (Cynefin classification, decomposition, exploration, evaluation, stress-test). Use it when the debug question becomes an architectural question and the user's context is needed.
...
```
## Handoff from debug-runtime → plan-tradeoff (Mode: Interactive)
...
### Re-entry after `plan-tradeoff` Interactive

If `plan-tradeoff` Interactive returns a recommendation that is a new architecture:
...
If `plan-tradeoff` Interactive returns "this is not a bug, it's a scope decision; defer":
...
2. **Shortcut-seeking.** `plan-tradeoff` (in either mode) is not a substitute for this skill's work. It is a genuine handoff for genuine stuck-points.
...
### To `plan-tradeoff` (Mode: Solo)

```
## Route: debug-runtime (Phase 2/3) → plan-tradeoff (Mode: Solo)
...
### What plan-tradeoff should produce
...
### To `plan-tradeoff` (Mode: Interactive)

```
## Route: debug-runtime (Fail 3) → plan-tradeoff (Mode: Interactive)
...
| Anti-pattern | Fix |
|---|---|
| Routing to `plan-tradeoff` (Solo) at Fail 1 | No — use the re-open-Phase-2 block first. Solo deep-think is for stuck evidence, not stuck hypotheses. |
| Routing to `plan-tradeoff` (Interactive) at Fail 2 | Try the re-open-Phase-1 block first. Fail 3 is the gate, not Fail 2. |
...
| Restarting `debug-runtime` from Phase 1 after any routed-out skill returns | Re-enter at Phase 2 (post-`plan-tradeoff`) or declare done (post-`audit-completion`). Never Phase 1 unless the symptom actually changed. |
