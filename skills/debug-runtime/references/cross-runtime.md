...
This skill runs on any runtime that supports an ask-user tool (needed at the 3-fails handoff fork, per `references/escalation.md`) or a prose fallback. Most debugging work is solo — the user is invoked only when the skill hands off to `plan-tradeoff` (Mode: Interactive).
...
2. The skill uses the runtime's ask-user tool to present 1-4 options:
   ...
   - Route to `plan-tradeoff` (Mode: Interactive — recommended at Fail 3)
...
questions: [
  {
    ...
    multiSelect: false,
    options: [
      { label: "Route to plan-tradeoff Interactive (Recommended)", description: "Hand off with the full 3-fails template; plan-tradeoff Interactive investigates the architectural question" },
...
- First option marked `(Recommended)` in label, based on the current state (at Fail 3, "Route to plan-tradeoff Interactive" is the recommended option)
...
**1. Route to plan-tradeoff Interactive (Recommended)**
Hand off with the 3-fails template; plan-tradeoff Interactive investigates the architectural
