---
name: research-powerpack-guide
description: "Comprehensive research methodology guide for coding agents using the Research Powerpack MCP server. Use this skill whenever you need to research a bug, choose a library, design architecture, investigate performance, understand language features, set up DevOps, audit security, integrate APIs, plan testing strategies, or solve frontend problems. Triggers on: research, investigate, why does this error, which library, how to design, best practice, compare options, debug, performance issue, security audit, what's the recommended approach, help me understand, migration guide, breaking change, should I use, community opinion, what do people think about, find alternatives. This skill turns a single-tool naive search into a multi-source validated research workflow."
---
...
**The core insight:**
No single tool is sufficient. `web_search` gives you breadth but not content.
...
|---|---|---|---|---|
| `web_search` | Breadth-first discovery | 3-50 keywords | URLs only (10 per keyword) | Always first. Find where answers live. |
| `search_reddit` | Community signal | 3-50 queries | Post titles + URLs | Find practitioner experiences, warnings, regrets. |
| `get_reddit_post` | Deep opinion mining | 2-50 URLs | Full comment threads | Extract exact recommendations, code, config values. |
| `deep_research` | Structured synthesis | 1-10 questions | 32K tokens of analysis | Complex questions needing multi-angle analysis. |
| `scrape_links` | Content extraction | 1-50 URLs | Structured content | Get actual page content from URLs found by other tools. |

### Tool Details

**web_search** — The starting gun for every research workflow.
- Runs 3-50 parallel Google searches. Recommended: 5-7 keywords.
- Returns ~10 URLs per keyword. 7 keywords = 70 candidate URLs.
- CRITICAL: Returns URLs only, never content. You MUST follow up with `scrape_links`.
...
**get_reddit_post** — The opinion deep dive.
- Fetches full comment threads from 2-50 Reddit URLs.
- Auto-allocates comment budget: 1000 total (2 posts = 500 each, 10 = 100 each).
- Set `use_llm=false` (the default) to get raw comments with exact quotes and code.
- Set `use_llm=true` ONLY when synthesizing 20+ posts and the user wants a summary.
...
**scrape_links** — The content extractor.
...
- Default `use_llm=true` strips navigation, ads, footers — returns only what you ask for.
- Best for: extracting structured data from docs, pricing pages, GitHub READMEs.
...
```
Step 1: web_search (5-7 keywords)
...
Step 2: scrape_links (top 3-5 URLs from Step 1)
...
|---|---|---|
| Runtime errors, compiler errors, crashes | `references/bug-fixing.md` | web_search -> scrape_links -> search_reddit |
| Library selection, migration, compatibility | `references/library-research.md` | web_search -> search_reddit -> scrape_links |
| System design, patterns, database choice | `references/architecture.md` | deep_research -> search_reddit -> get_reddit_post |
| Async, type systems, macros, FFI, idioms | `references/language-idioms.md` | web_search -> deep_research -> get_reddit_post |
| CI/CD, Docker, K8s, monitoring, infra | `references/devops.md` | web_search -> scrape_links -> search_reddit |
| OWASP, auth, encryption, input validation | `references/security.md` | deep_research -> scrape_links -> web_search |
| Profiling, query optimization, caching | `references/performance.md` | deep_research -> search_reddit -> scrape_links |
| Third-party APIs, webhooks, OAuth, payments | `references/api-integration.md` | web_search -> scrape_links -> deep_research |
| Test frameworks, mocking, E2E, load testing | `references/testing.md` | search_reddit -> deep_research -> web_search |
| Framework migration, SSR/SSG, accessibility | `references/frontend.md` | web_search -> search_reddit -> scrape_links |
...
- **NEVER stop at web_search.** It returns URLs, not content. Always follow with `scrape_links`.
...
- **Follow up** with `get_reddit_post` on the 5-7 highest-signal threads.
...
- **Pair with web_search results.** The standard workflow is: web_search -> pick URLs -> scrape_links.
...
- Using ONLY `web_search` and stopping. You have URLs but no content and no validation.
  Fix: Always follow `web_search` with `scrape_links`, and add `search_reddit` for validation.
...
**URL Hoarding**
- Running `web_search` and collecting 70 URLs but never scraping any of them.
  Fix: Pick the top 3-5 most relevant URLs and run `scrape_links` immediately.
...
**Losing Exact Values**
- Using `get_reddit_post` with `use_llm=true` when you need exact config values or code.
...
- Treating `deep_research` output as fully cited fact. It can hallucinate sources.
  Fix: When a claim matters, verify it by running `web_search` + `scrape_links` on the source.
...
**Over-Researching**
- Running all 5 tools for a simple factual question that `web_search` + `scrape_links` answers.
...
`web_search` (find URLs) -> `scrape_links` (get content) -> validate with one more tool.
