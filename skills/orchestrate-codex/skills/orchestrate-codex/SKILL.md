...
- **`--apply ids:` bypasses status filters.** `rescue --apply ids:s1,s2,s3` is operator-by-name and ignores status; a `done` entry named in `ids:` flips to `queued` and redispatches. Status-filter subsets (`failed-only` / `never-started-only` / `all-non-done`) only apply to those named variants. Why: `ids:` is the operator-override path by design — sometimes you want to redo a "done" entry. Workaround: combine `ids:` with explicit knowledge of entry status; for status-aware filtering, use the named-subset variants. See `references/modes/rescue.md`.
