...
|---|---|---|---|---|---|
| exec | `preflight.sh` (manual) | `orchestrate-codex.mjs exec` → `run-fleet.sh` (uses `setup-worktree.sh` per task) | `codex-monitor.sh` | runners call `python3 manifest-update.py --execute` (via `_lib.sh::oc_manifest_set`); audit via `audit.py state` | `cleanup-worktrees.py` |
| batch | `preflight.sh` (manual) | `orchestrate-codex.mjs batch` → `render.sh --mode template` → `run-batch.sh` | `codex-monitor.sh --tail-runner-log` | `audit.py {state,sizes}` | (no worktrees to tidy; manifest deleted) |
| single | `preflight.sh` (manual) | `orchestrate-codex.mjs single` → `run-single.sh` (uses `codex-json-filter.sh`) | filter pipe is the monitor | manifest writes through Python | `cleanup-worktrees.py` only when prior exec/review worktrees need cleanup |
| review | `preflight.sh` (auto) | `orchestrate-codex.mjs review` → `run-review.sh` (uses `setup-worktree.sh`, `classify-review-feedback.py`) | `codex-monitor.sh` | manifest writes through Python | `cleanup-worktrees.py` |
| rescue | n/a for classify; original mode on redispatch | `orchestrate-codex.mjs rescue` → `rescue-detect.py`; `--apply ...` → original mode's runner | inherits original mode | manifest writes through Python | inherits original mode |
| audit | n/a | `orchestrate-codex.mjs audit` → `audit.py state` | n/a (read-only) | n/a | n/a |
