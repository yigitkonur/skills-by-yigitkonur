...
| Rescue dispatcher classifies but does not re-spawn | `node orchestrate-codex.mjs rescue` returns an envelope with classification + `next_action` prose; nothing flips statuses, nothing calls `spawnRunnerDetached` | C1/C9 derailments confirmed `handleRescue` is half-implemented. Until the dispatcher is wired up, after rescue classification the operator must manually: (1) flip selected entries via `python3 manifest-update.py entry --set status=queued --set finished_at=null --set exit_code=null --execute`, (2) clean up orphan codex procs (`pgrep -f 'codex exec'`), (3) directly invoke `bash scripts/run-<mode>.sh "$MANIFEST"` with the appropriate `JOBS=` env. The skill's spine documents this as automatic; the runtime currently does not. Treat this row as a known gap — sibling agent mission targets it. |
