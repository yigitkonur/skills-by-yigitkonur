...
```bash
ORCHESTRATE_CODEX_MODEL=gpt-5.6 \
ORCHESTRATE_CODEX_EFFORT=high \
node scripts/orchestrate-codex.mjs exec --tasks tasks.json
```
...
The `codex-companion.mjs` dispatcher recognizes one alias: `spark` → `gpt-5.3-codex-spark`. The skill does not surface this alias; if you want spark, pass `ORCHESTRATE_CODEX_MODEL=gpt-5.3-codex-spark` directly.
