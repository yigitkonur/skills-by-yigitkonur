...
2. **Subagent does the using. You do the detective work.** Sonnet follows the skill. You read the trace and fix.
...
Spin up a **Sonnet** subagent. The prompt should read like a real user request.
...
As you work, just flag anything that trips you up:
- [STUCK] if you genuinely can't figure out what to do next
- [GUESSED] if you had to assume something the skill didn't say
- [BROKE] if a command or pattern from the skill didn't work
- [NICE] if the skill saved you from a mistake
...
For each friction point, tag the root cause using `references/root-cause-taxonomy.md`.

### 5. Fix the skill directly

For each friction point, highest severity first:

1. Match to a fix pattern from `references/fix-patterns.md`
2. Edit the skill's source files (SKILL.md or the relevant reference)
3. Fixes: in-place, self-contained, minimal
...
If round 1 found any P0, launch another Sonnet with a **different task in a different domain**. Max 3 rounds. If friction isn't decreasing after 3, the skill needs architectural redesign.
