...
| `USE_CODEX_MANIFEST` | none | Path to the use-codex manifest. Without it, the monitor only reports git/process counts (no manifest summary). |
| `MONITOR_ROOT` | `/tmp/use-codex-monitor` | Logs dir; `monitor.log` and `baseline.sha` land here |
