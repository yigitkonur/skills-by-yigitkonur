#!/usr/bin/env python3
"""Classify use-codex manifest entries for rescue-mode operator review.
...
Read-only. Outputs JSON for the dispatcher (use-codex.mjs) or human
...
`use-codex.mjs rescue --redo failed|never-started|all-non-done`.
...
def main() -> int:
    ...
    ap.add_argument("--manifest", required=True, help="Path to use-codex manifest.json")
    ...
    ap.add_argument("--json", action="store_true", help="Emit JSON output for use-codex.mjs.")
    args = ap.parse_args()
