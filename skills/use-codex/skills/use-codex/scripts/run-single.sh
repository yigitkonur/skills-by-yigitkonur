...
# use-codex.mjs:2210-2219. Default unset = fresh `codex exec`.
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    # use-codex.mjs records the schema path in the manifest entry
    ...
    # Rescue context-preserving resume. use-codex.mjs:2210-2218 picks one
...
if [[ -z "$PROMPT_FILE" && -n "$PROMPT_TEXT" ]]; then
  mkdir -p "$CWD/.use-codex"
  PROMPT_FILE="$CWD/.use-codex/${USE_CODEX_ENTRY_ID}.prompt.md"
...
if [[ -z "$OUT" ]]; then
  OUT="$CWD/.use-codex/single.last.md"
fi
if [[ -z "$JSONL" ]]; then
...
if [[ "$DRY_RUN" == "1" ]]; then
  ...
  printf '{"type":"use-codex.done","entry_id":"%s","status":"done"}\n' "$USE_CODEX_ENTRY_ID" >> "$LOG_PATH"
...
# Sentinel writer: appends one use-codex.done JSONL event to the live log
...
emit_orchestrate_done() {
  local status="$1"
  if [[ -n "$LOG_PATH" ]]; then
    printf '{"type":"use-codex.done","entry_id":"%s","status":"%s"}\n' \
