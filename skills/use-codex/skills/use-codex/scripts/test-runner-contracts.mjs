...
const DISPATCHER = path.join(SCRIPT_DIR, "use-codex.mjs");
...
function tmpdir(name) {
  return fs.mkdtempSync(path.join(os.tmpdir(), `use-codex-contract-${name}-`));
}
...
// at use-codex.mjs:1474 persists `answers_dir` to manifest.answers_dir
...
function scenarioSentinelParity() {
  ...
  // the use-codex.done JSONL into `--- single done (single: done) ---`.
  ...
  if (sLog && fs.existsSync(sLog)) {
    ...
    // Verify the use-codex.done sentinel was appended to JSONL for filter translation
    const jsonlPath = sOut.json?.result?.jsonl_path;
    if (jsonlPath && fs.existsSync(jsonlPath)) {
      const j = fs.readFileSync(jsonlPath, "utf8");
      expect(j.includes('"type":"use-codex.done"'),
        "single runner appends use-codex.done to JSONL for filter translation");
    }
  } else {
...
function scenarioSymlinkInvocationInline() {
  ...
  const real = path.join(SCRIPT_DIR, "use-codex.mjs");
...
// use-codex.mjs handleRescue (between corruption and freshness gates)
