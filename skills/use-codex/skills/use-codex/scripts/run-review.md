...
Positional, simple by design. There are NO `--manifest`, `--base`, `--concurrency`, `--max-rounds`, `--state-dir`, or `--only` flags — they were documented in earlier drafts but never implemented; the dispatcher invokes the runner with just `[manifestPath, "1"]` plus optional `--dry-run` (`use-codex.mjs:1811-1813`).
