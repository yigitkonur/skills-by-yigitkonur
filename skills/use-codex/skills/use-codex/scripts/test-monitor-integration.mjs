...
function scenarioCarryForwardDoneEntries() {
  ...
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "use-codex-carry-forward-"));
...
async function main() {
  process.stdout.write("test-monitor-integration.mjs — use-codex Monitor + dispatcher harness\n");
  scenarioExec();
  scenarioBatch();
  scenarioSingle();
  scenarioReview();
