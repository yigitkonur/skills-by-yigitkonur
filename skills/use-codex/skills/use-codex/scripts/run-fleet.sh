...
run_one() {
  ...
  # run logs dir) and fall back to the worktree's .use-codex dir if
  ...
  if [[ -n "$monitor_root" ]]; then
    ...
    base_dir="$wt_resolved/.use-codex"
  fi
  ...
  if [[ "$DRY_RUN_LOCAL" == "1" ]]; then
    ...
    # carryForwardDoneEntries (use-codex.mjs) to skip this entry on
