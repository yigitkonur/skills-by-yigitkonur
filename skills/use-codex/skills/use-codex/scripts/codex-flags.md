...
| Var | Default | Effect |
|---|---|---|
| `USE_CODEX_CODEX_MODEL` | `gpt-5.5` | Override the pinned model |
| `USE_CODEX_CODEX_EFFORT` | `xhigh` | Override the pinned `model_reasoning_effort` |
...
- Idempotent: sourcing twice is a no-op (`USE_CODEX_CODEX_FLAGS_LOADED=1` guard).
