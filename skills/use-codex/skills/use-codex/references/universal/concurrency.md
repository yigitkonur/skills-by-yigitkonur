...
4. If 503s persist after halving twice, the auth tier is the bottleneck — no concurrency tuning fixes it. Wait longer, pause sibling Claude / use-codex sessions sharing the same auth tier, or switch tiers. See `references/universal/failure-modes.md` row 1 for the multi-session auth-sharing note.
...
`JOBS > 100` is refused unconditionally. The skill assumes a single user's auth tier; concurrency above 100 is structurally pathological for this skill. If you genuinely need it, you're not running use-codex — you're running a queue worker.
