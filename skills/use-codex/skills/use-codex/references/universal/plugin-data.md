...
Every use-codex run writes its manifest under the same state directory algorithm used by vendored codex-cc. Codex-companion uses the same workspace-slug+hash function, so our manifest sits next to its `state.json` and `jobs/` records — rescue can correlate by codex thread id.
...
```
state-root = "<resolved-state-dir>/use-codex"
...
### Symmetric fallback — use-codex and codex-companion agree

Both writers — use-codex (dispatcher + `bootstrap.sh`) and the vendored codex-companion (`scripts/codex-cc/lib/state.mjs`) — share the same resolver chain. There is no asymmetry between the two.
...
**Consequence:** rescue's correlation between manifest entries (use-codex's `manifest.json`) and codex-companion job records (`state.json`, `jobs/<id>.json`) holds whenever both writers ran in the same process tree, because they computed the same `<slug>-<hash>` against the same `${CLAUDE_PLUGIN_DATA}` (set or unset). The cross-link breaks only when `${CLAUDE_PLUGIN_DATA}` flips partway through the session — e.g. set on dispatch but unset for a later helper invocation, so the two writers land in different roots.

**Operators should export `${CLAUDE_PLUGIN_DATA}` once for the session OR always invoke `node use-codex.mjs <mode>` (which goes through `bootstrap.sh`) rather than the helper scripts directly.** Switching the env var mid-session is the only way the two roots can diverge.
...
What use-codex itself writes:

```
<state-root>/
├── manifest.json                    # the use-codex manifest
└── manifest.json.lock               # advisory lock file (flock)
```

That's it. The skill writes no other state files at this level — no jobs/, no broker.json, no per-run logs (logs land under `<monitor-root>/logs/<run-id>/`, configurable via `MONITOR_ROOT` env, default `/tmp/use-codex-monitor/`).
...
```
<workspace-slug>-<hash>/
├── use-codex/manifest.json       # this skill's manifest
├── use-codex/manifest.json.lock
...
Note the slight nesting offset: use-codex's manifest is in `<state-root>/manifest.json` where `<state-root>` ends in `/use-codex/`. Codex-companion's `state.json` is one directory up, in `<workspace-slug>-<hash>/state.json`. Both writers anchor on the same `<workspace-slug>-<hash>` regardless of whether `${CLAUDE_PLUGIN_DATA}` is set, because they share the same resolver chain (see "Symmetric fallback" above). Rescue's correlation breaks only if `${CLAUDE_PLUGIN_DATA}` flips between writes — see also `references/universal/manifest-contract.md` §Path.
...
To manually prune all use-codex state for a workspace:
...
```bash
ls -t <workspace-slug>-<hash>/use-codex/manifest.*.json 2>/dev/null | tail -n +2 | xargs rm -f
```

The skill's tidy command (`node use-codex.mjs tidy`) does NOT remove the state directory; it only removes worktrees and clears the active manifest.
...
ls -la "${CLAUDE_PLUGIN_DATA:-...}/state/<slug>-<hash>/use-codex/"
```
