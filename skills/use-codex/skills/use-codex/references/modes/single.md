...
The dispatcher's full `valueOptions` for single mode are: `prompt`, `prompt-file`, `out`, `cwd`, `monitor-root`, `run-id`, `output-schema`, `resume-thread`. Boolean options: `reuse-worktree`, `resume-last`, `force-new-run`, `dry-run` (see `handleSingle` in `scripts/use-codex.mjs`). Any unrecognized flag is rejected by the strict parser.
...
- **Reuse** an existing worktree (deps installed, specific branch, half-finished work): `cd ../myrepo-wt-feat-auth && node use-codex.mjs single --prompt-file ./prompt.md --cwd "$PWD"`.
...
1. **Capture `runner_pid` from the dispatcher envelope.** When `node use-codex.mjs single` returned, its envelope's `result.runner_pid` is the bash runner's PID. If you no longer have the envelope, `pgrep -f 'run-single.sh.*<entry-id>'` or `pgrep -f 'codex exec.*<cwd>'` will find it.
