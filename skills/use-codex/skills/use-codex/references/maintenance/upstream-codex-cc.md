...
node <skill-root>/scripts/use-codex.mjs --help
...
The dispatcher `use-codex.mjs` imports these explicitly. After an update:
1. Re-run `node --check <skill-root>/scripts/use-codex.mjs` — catches stale imports.
...
- Identify the replacement; update the import in `use-codex.mjs`.
