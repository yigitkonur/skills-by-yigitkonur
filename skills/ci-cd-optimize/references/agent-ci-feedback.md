...
2. **Terminal on every path.** `success`, `failure`, `no-run`, `superseded`, `timeout`, `probe-dead`. Silence past the deadline must be structurally impossible, not merely unlikely.
...
Enumerate all six; a missing one is a hang waiting to happen. `no-run` and `superseded` are the two most often omitted and the two that most often masquerade as "CI is just slow".
...
| `CI-DONE timeout`/`probe-dead`/`superseded` | the watcher gave up safely — re-check manually, never assume fine |
...
- **One watch per pushed identifier.** A re-push supersedes the old run; let the old watcher exit `superseded` and arm a new one rather than reasoning about two overlapping streams.
