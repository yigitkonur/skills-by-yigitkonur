...
If you keep a full dependency-tree cache, key it on the manifest **and** the lockfile. Keyed on the lockfile alone it can hit after a manifest-only change and silently test a stale tree.

### Time the uncached path before adding any cache

"Installs are slow" is an assumption until measured. Several CI providers run a pull-through registry proxy on the runner network and point the package manager at it automatically; where that exists, a store cache can be redundant, paying restore and save cost to avoid a fetch that was never slow. One observed A/B on a fixed commit saw ~5s installs both with and without a full `node_modules` cache.

Test it directly rather than reasoning about it:

1. Find a run where the store cache missed (a new lockfile hash, or a first run on a branch).
2. Compare its install step against a warm-cache run.

If install is roughly the same in both, the proxy is doing the work and the cache is only buying its own overhead. If the cold install is materially slower, keep the cache. Do not infer either way from cache hit counts alone — a cache can report a high hit rate while saving nothing, which is the failure mode this whole file exists to prevent. When you remove a cache, report the numbers, because "we deleted the cache" reads as a regression to anyone who does not see the break-even.
