...
gh api repos/OWNER/REPO \
  --jq '{repo: .full_name, stars: .stargazers_count, forks: .forks_count, pushed: .pushed_at[:10], archived: .archived, disabled: .disabled, license: (.license.spdx_id // "none"), open_issues: .open_issues_count, default_branch: .default_branch}'
```
...
gh api 'repos/OWNER/REPO/releases?per_page=3' \
  --jq '.[] | "\(.tag_name)	\(.published_at[:10])	\(.name // "")"'
```
...
gh api repos/OWNER/REPO/community/profile \
  --jq '{health: .health_percentage, readme: (if .files.readme then true else false end), contributing: (if .files.contributing then true else false end), license: (.files.license.spdx_id // "none")}'
```
