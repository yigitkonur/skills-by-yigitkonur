...
Standard brainstorming questions for the planning phase. Ask the 5 core questions (Q1-Q5) for every non-trivial plan, then add at most 2 optional follow-ups when the context demands it. Users answer in compact form: `1b,2d,3g,4a,5c`.

## Core question rule

Always ask:
- Q1: scope/type
- Q2: decomposition depth
- Q3: wave count
- Q4: success criteria
- Q5: execution style

Optional follow-up selection:

| Context | Optional follow-ups |
|---|---|
| New feature in existing project | Q6, Q7 |
| Refactor or migration | Q7, Q8 |
| Bug fix campaign | Q9 |
| Infrastructure or DevOps | Q10 |
| Greenfield project | none by default |
...
```
a) Light — Epics → Tasks (2 levels, only for trivial or tightly bounded work)
b) Standard — Epics → Features → Tasks (3 levels, minimum for non-trivial plans)
...
### Q3: How many post-foundation execution waves?

Wave 0 foundation is always present. The choices below count the waves after Wave 0.

```
a) 1 post-foundation wave — foundation → delivery
b) 2 post-foundation waves — foundation → core → polish
c) 3 post-foundation waves — foundation → core → advanced → release
d) 4 post-foundation waves — foundation → core → integrations → advanced → release
e) 5 post-foundation waves — foundation → core → integrations → advanced → hardening → release
f) 6+ post-foundation waves — complex multi-phase (state the highest wave number in follow-up)
g) Manual — I will specify the post-foundation wave structure
```

If Q3 implies Wave 6 or higher, set `MAX_WAVE` to that highest wave number before running `scripts/setup-labels.sh`.
...
Present Q1-Q5 first. Add optional follow-ups only if they materially change the plan. Number them sequentially. If the user replies in prose, restate your normalized Q1-Q5 mapping before you design the tree. End with:
