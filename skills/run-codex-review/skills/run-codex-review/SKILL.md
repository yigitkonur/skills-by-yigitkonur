...
- The user wants generic non-codex parallel coding fan-out → load `orchestrate-codex` exec mode directly
...
|---|---|---|
| Per-branch codex review fix loop (branch list + review keyword) | `orchestrate-codex` review mode | `node skills/orchestrate-codex/skills/orchestrate-codex/scripts/orchestrate-codex.mjs review --branches <list>` |
| Resume / rescue an interrupted codex review run | `orchestrate-codex` rescue mode | `node skills/orchestrate-codex/skills/orchestrate-codex/scripts/orchestrate-codex.mjs rescue --manifest <path>` |
...
1. **Codex review ≠ PR review.** `orchestrate-codex` review mode runs `codex exec review --base <base>` per branch and converges via classifier rounds. It does NOT open PRs. PR creation is `ask-review`. Reviewer-side merge calls are `do-review`.
2. **Branch list signal wins for fix loops.** If the user supplies ≥2 branches AND mentions "review/ship/merge/close out", route to `orchestrate-codex` review — even if the prompt also mentions PRs. PR handoff is a *separate* downstream step routed to `ask-review` after convergence.
...
4. **Manifest existence forces rescue.** If `${CLAUDE_PLUGIN_DATA}/state/<slug>-<hash>/orchestrate-codex/manifest.json` (or the codex-companion fallback under `${TMPDIR:-/tmp}/codex-companion`) exists with non-terminal entries AND the user says "resume / continue / pick up / rescue / prior run", route to `orchestrate-codex` rescue. Do not overwrite a live manifest.
...
6. **Slash-command boundary.** The vendored `/codex:review` lives inside `orchestrate-codex` under `scripts/codex-cc/` and is not used by review mode. Review mode uses native `codex exec review`. There is no slash command owned by *this* shim.

## Trigger-precision cues for the canonical route

When `orchestrate-codex` is the route (review or rescue mode), confirm these environment signals before handing off so the canonical skill does not bounce the request:
...
- `codex login status` is a soft probe; bearer-token / managed-companion / proxy auth is allowed even when status reports "Not logged in" (set `ORCHESTRATE_SKIP_CODEX_AUTH=1` to bypass the bash bootstrap hard-fail)
- For rescue: a manifest path exists under the resolved state dir and `manifest.mode == "review"` for review-fleet rescue

If any cue fails, surface the gap in one line and stop — do not auto-improvise into `orchestrate-codex` and let it fail downstream.
...
- Never collapse "codex converges branches" and "PR is ready for review" into one skill call — they are two routes (`orchestrate-codex` review → `ask-review`).
- Never invent a `/codex:resc` or similar slash command. Rescue is a CLI subcommand of `orchestrate-codex`, not a slash command of this shim.
