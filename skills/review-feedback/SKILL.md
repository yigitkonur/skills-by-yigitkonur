...
- *performing the review yourself* (reviewer-side, producing findings) → use `review-pr`
- *preparing a PR for review* (author-side, asking for feedback) → use `review-self`
- *checking what work is done vs. claimed done* with no review feedback in hand → use `audit-completion`
- *running `run-codex-2` per-branch convergence loops* — major-finding evaluation routes through `review-pr`, not this skill
