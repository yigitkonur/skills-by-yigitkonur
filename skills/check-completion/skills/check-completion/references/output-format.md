...
<Bulleted list of rows ending in `Deferred to Human` / `Deprioritized` / `Cancelled` / `Out of Scope` / `Superseded` / `Blocked — unresolvable`, with rationale>
```
...
- **Deferred / deliberate stops** — every terminal non-`Implemented` row, with rationale. Shows the user what's explicitly handed off, deprioritized, cancelled, out of scope, superseded, or blocked.
