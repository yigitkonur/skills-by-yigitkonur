...
This skill stays small on purpose. It is the generic, CLI-agnostic batch fanout pattern. If the run is codex-only and needs worktrees, manifests, monitor events, or one of the other codex modes (exec / single / review / rescue), reach for `run-codex-2` instead.
...
|---|---|
| One discrete codex invocation, no list. | `run-codex-exec` (deprecated compat shim → `run-codex-2` exec mode) |
...
| Codex-only fleet with worktrees, manifests, Monitor events, or rescue mode. | `run-codex-2` |
...
| `run-codex-2` | codex-only fleet, worktrees, Monitor events, manifest, rescue mode | full codex runtime contract; this skill is the lighter CLI-agnostic cousin |

If the request says "codex" plus "worktree" or "manifest" or "Monitor", route to `run-codex-2`. If it just says "run this prompt over each item in the list," stay here.
