...
After the Makefile is generated, the skill normalizes the project's agent docs as the very last step (before the optional CI/CD wiring). The contract is fixed: **`AGENTS.md` is the source of truth; `CLAUDE.md` is a symlink → `AGENTS.md` (or absent).** This is a deliberate convention — one place to edit, two places agents look. Five starting states cover every observed shape of the world.

This file is the only one in this skill that reads existing prose, diffs it, and asks the user a question — every other reference defines deterministic transformations of generated code. Treat AGENTS.md normalization with corresponding care.
...
| Working tree dirty for `AGENTS.md` | `git status --porcelain AGENTS.md` non-empty | Warn but proceed (the user might be mid-edit). The wipe-and-regen rule already snapshotted the working tree before this step. |
...
- **DO NOT touch AGENTS.md if `## Make targets` was last hand-written by the user (no skill-generated lead paragraph).** Ask before replacing user prose.
...
| The skills repo's own AGENTS.md (orthogonal — that's a different skill, `init-agent-config`) | cross-skill: `init-agent-config` |
