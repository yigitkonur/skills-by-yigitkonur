...
- *the task is design, architecture, or refactor with no reproducible runtime bug* → `do-think`
- *the user explicitly wants user-in-the-loop brainstorm or co-authored reasoning* → `do-think` (Mode: Interactive)
...
- *only "confirm what's actually done" on already-fixed items is needed* → `check-completion`
...
Boundary with `do-think`: three failed fixes routes here to `do-think` (Mode: Interactive) with the handoff template. After `do-think` returns, re-enter `do-debug` at **Phase 2** with the new framing — never Phase 1.
...
5. **Escalate at 3.** Three failed fixes triggers the `do-think` Interactive handoff (Escalation gate, below).
...
4. **All candidates falsified** → route to `do-think` to re-form the candidate space, then re-enter Phase 2.
...
5. **Hand off to `check-completion`** — verify nothing else in scope was broken. See `references/integration.md` for the handoff format.
...
| Phase 2: only "it feels like X" as evidence | Route | `do-think` (`workflows/bug-tracing.md`) | Stronger reasoning loop re-grounds the investigation |
| Phase 3: 2–3 candidates look equally plausible | Route | `do-think` foundations / evidence-and-falsification | Stronger evidence framework before the experiment |
...
| Fail #3 — **stop fixing** | Route | `do-think` (Mode: Interactive); template in `references/integration.md` | Architecture-shaped; re-enter Phase 2 after return |
| Phase 4: fix applied, verification passed | Route | `check-completion` | Audit for related-but-forgotten scope |
| "Bug" is a design disagreement, not a bug | Don't start | `do-think` (Solo or Interactive) | Not a runtime failure |
...
- **Fail 3** → **Stop fixing.** Route to `do-think` (Mode: Interactive). Three failures means the problem is architecture-shaped, not a bug. Handoff format: `references/escalation.md` and `references/integration.md`. After `do-think` returns, re-enter at Phase 2 with the new framing.
...
| `references/integration.md` | Unsure whether to stay in-skill or route to `do-think` / `check-completion` |
...
| Fail #1–#2 → re-open prior phases; Fail #3 → route to `do-think` (Interactive) | Try fix #4, #5, #6 |
...
- Do not recommend `do-debug` as the next step (no infinite regress). State which phase to re-enter.
...
- **3 fails hit** → hand off to `do-think` (Mode: Interactive) using the template in `references/integration.md`; do not retry.
