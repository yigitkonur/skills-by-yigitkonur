...
This skill coordinates with three siblings in the pack: `do-think` (solo reasoning), `do-brainstorm` (user-in-loop framework routing), `check-completion` (audit-then-remediate). The inline routing matrix in SKILL.md covers the common cases; this file covers edge cases and the handoff templates.
...
- **Stuck on evidence** (Phase 2 pattern can't form) → route to `do-think`
...
## Route to `do-think` when...

`do-think` is the pack's tool-agnostic solo-reasoning skill. It does not require user input (unlike `do-brainstorm`) and applies a stronger evidence-and-falsification loop than Phase 2/3.
...
- You are at the 3-fails gate. That's `do-brainstorm` territory, not `do-think`.

### Worked handoff examples

**Example A** — Phase 2 candidates all weak:

> Handing to `do-think`. Phase 1 is solid (symptom card + 10/10 repro). Phase 2 produced three candidate mechanisms, all rated "maybe": (1) race on the session store, (2) middleware order issue, (3) cache invalidation timing. None has evidence stronger than "it could be this." Need a stronger evidence frame before running Phase 3.

**Example B** — Phase 3 tied:

> Handing to `do-think`. Two candidates fit: (1) connection pool exhaustion under load, (2) DNS resolution timing out on the cold path. Both would produce the observed "intermittent slow requests + error log." Tests so far have not distinguished them. Need a design for a falsifying experiment.

### Re-entry after `do-think`
...
2. **You are tired and looking for a shortcut.** `do-think` and `do-brainstorm` are not substitutes for the work you should be doing in this skill. They are genuine handoffs for genuine stuck-points.
...
```
## Route: do-debug (Phase 2/3) → do-think
...
```
## Route: do-debug (Fail 3) → do-brainstorm
...
| Anti-pattern | Fix |
|---|---|
| Routing to `do-think` at Fail 1 | No — use the re-open-Phase-2 block first. `do-think` is for stuck evidence, not stuck hypotheses. |
...
| Restarting `do-debug` from Phase 1 after any routed-out skill returns | Re-enter at Phase 2 (post-`do-think`/`do-brainstorm`) or declare done (post-`check-completion`). Never Phase 1 unless the symptom actually changed. |
